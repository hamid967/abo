...
describe("سير بناء Android المستقل", () => {
  ...
  it("يفرض أسرار التوقيع ويصدر APK أو AAB موقعة كـArtifact", () => {
    ...
    expect(workflow).toContain("-PABU_MISHAL_KEYSTORE_FILE=abu-mishal-release.jks");
  ...
  it("يحافظ على هوية حزمة Android ويقبل رقم إصدار من سير العمل", () => {
    ...
    expect(gradle).toContain('providers.gradleProperty("ABU_MISHAL_VERSION_CODE")');
    expect(gradle).toContain('providers.gradleProperty("ABU_MISHAL_VERSION_NAME")');
  });
});
