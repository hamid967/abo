        ...
        {!isAuthenticated ? (
          ...
          <>
            ...
            <View style={[styles.filters, { flexDirection: isArabic ? "row-reverse" : "row" }]}>
              {(["all", "unread", "tasks"] as NotificationFilter[]).map((value) => <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: filter === value }} onPress={() => setFilter(value)} style={[styles.filter, filter === value && styles.filterActive]}><Text style={[styles.filterText, filter === value && styles.filterTextActive, { writingDirection: direction }]}>{({ all: text.all, unread: text.unread, tasks: text.tasks })[value]}</Text></Pressable>)}
            </View>
            <FlatList
              data={items}
              ...
              renderItem={({ item }) => {
                ...
                return <Pressable accessibilityRole="button" accessibilityLabel={item.task ? `${item.title}. ${text.openTask}` : item.title} onPress={() => void openNotification(item)} style={({ pressed }) => [styles.card, !item.readAt && styles.unread, pressed && styles.pressed]}>
