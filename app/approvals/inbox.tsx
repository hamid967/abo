...
export default function ApprovalsInboxScreen() {
  ...
  const inbox = trpc.approvals.inbox.useQuery(undefined, { enabled: isAuthenticated });
  ...
  const text = isArabic ? { title: "صندوق وارد الموافقات", subtitle: "قراراتك المعلقة فقط، مرتبة من الأقدم للأحدث", signIn: "سجّل دخولك للوصول إلى الموافقات المسندة لك", empty: "ما عندك موافقات معلقة حالياً.", pending: "معلّق", expired: "منتهٍ", step: "خطوة الاعتماد", note: "ملاحظة اختيارية للمسار", open: "فتح متابعة المهمة", today: "معلّق" } : { title: "Approvals inbox", subtitle: "Only your pending decisions, oldest first", signIn: "Sign in to access approvals assigned to you", empty: "You have no pending approvals.", pending: "Pending", expired: "Expired", step: "Approval step", note: "Optional note for the workflow", open: "Open task tracking", today: "Pending" };

  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.container}>
    <View style={[styles.header, { flexDirection: isArabic ? "row-reverse" : "row" }]}><Pressable accessibilityLabel={isArabic ? "رجوع" : "Back"} onPress={() => router.back()} style={styles.close}><Ionicons name="arrow-forward" color="#17382F" size={21} /></Pressable><View style={styles.headerCopy}><Text style={[styles.title, { writingDirection: direction }]}>{text.title}</Text><Text style={[styles.subtitle, { writingDirection: direction }]}>{text.subtitle}</Text></View></View>
    {!isAuthenticated ? <Pressable onPress={() => router.push("/account" as never)} style={styles.empty}><Ionicons name="lock-closed-outline" size={30} color="#0B5D45" /><Text style={[styles.emptyText, { writingDirection: direction }]}>{text.signIn}</Text></Pressable> : inbox.isLoading ? <ActivityIndicator color="#0B5D45" style={styles.loading} /> : <>
      <View style={[styles.summary, { flexDirection: isArabic ? "row-reverse" : "row" }]}><Metric icon="time-outline" value={(inbox.data ?? []).length} label={text.today} direction={direction} /><Metric icon="alert-circle-outline" value={expired} label={text.expired} direction={direction} /></View>
