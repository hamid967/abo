...
export default function AbuMishalChatScreen() {
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.container}>
    ...
    <FlatList data={messages} keyExtractor={(item) => String(item.id)} contentContainerStyle={styles.list} ListEmptyComponent={mine.isLoading || detail.isLoading ? <ActivityIndicator color="#0B5D45" style={styles.loading} /> : <View style={styles.empty}><Ionicons name="hand-left-outline" size={31} color="#0B5D45" /><Text style={[styles.emptyText, { writingDirection: direction }]}>{text.empty}</Text></View>} renderItem={({ item }) => {
