...
type ChatMessage = { id: string; role: "user" | "assistant"; text: string; createdAt: string; deliveryStatus: "sent" | "delivered" | "failed"; sources?: Source[]; confidence?: "high" | "medium" | "low"; followUpQuestions?: string[]; suggestedAction?: "none" | "open_request" | "track_transaction" | "open_support" };
type CopiedResponse = { id: string; text: string; copiedAt: string };
type AssistantSuggestion = { label: string; action: "ask" | "request" | "intake" | "support" };
...
export default function AssistantScreen() {
  ...
  const suggestions: AssistantSuggestion[] = isArabic ? [
    { label: "ما حالة معاملتي؟", action: "ask" }, { label: "ما المستندات الناقصة؟", action: "ask" }, { label: "لخّص آخر تحديثات معاملتي.", action: "ask" }, { label: "أريد الاستفسار عن خدمة.", action: "ask" }, { label: "عبّئ لي طلباً عبر المحادثة.", action: "intake" }, { label: "أنشئ لي طلباً بالنموذج.", action: "request" }, { label: "أريد التواصل مع الموظف.", action: "support" },
  ] : [
    { label: "What is my transaction status?", action: "ask" }, { label: "Which documents are missing?", action: "ask" }, { label: "Summarise my latest transaction updates.", action: "ask" }, { label: "I want to ask about a service.", action: "ask" }, { label: "Fill a request by chat.", action: "intake" }, { label: "Create a request with the form.", action: "request" }, { label: "I want to contact the team.", action: "support" },
  ];
  ...
  const send = async (suggestedQuestion?: string) => {
    ...
    try {
      ...
      setMessages((current) => [...current, { id: `${messageId}-answer`, role: "assistant", text: response.answer, createdAt: new Date().toISOString(), deliveryStatus: "delivered", sources: response.sources, confidence: response.confidence, followUpQuestions: response.followUpQuestions, suggestedAction: response.suggestedAction }]);
    } catch {
    ...
    {!isAuthenticated ? <Pressable onPress={() => router.push("/account" as never)} style={styles.empty}><Ionicons name="lock-closed-outline" size={32} color="#0B5D45" /><Text style={[styles.emptyText, { writingDirection: direction }]}>{text.signIn}</Text></Pressable> : <>
      <ScrollView contentContainerStyle={styles.messages} showsVerticalScrollIndicator={false}>
        ...
        {messages.map((message) => <View key={message.id} style={[styles.message, message.role === "user" ? styles.userMessage : styles.assistantMessage]}>
          ...
          {message.role === "assistant" && message.confidence ? <View style={styles.intelligencePanel}><Text style={[styles.confidenceText, { writingDirection: direction }]}>{isArabic ? `درجة الثقة: ${message.confidence === "high" ? "عالية" : message.confidence === "medium" ? "متوسطة" : "تحتاج تحقق"}` : `Confidence: ${message.confidence}`}</Text>{message.followUpQuestions?.map((item) => <Pressable key={item} onPress={() => setQuestion(item)} style={styles.followUpChip}><Ionicons name="sparkles-outline" size={13} color="#007A59" /><Text style={[styles.followUpText, { writingDirection: direction }]}>{item}</Text></Pressable>)}{message.suggestedAction && message.suggestedAction !== "none" ? <Pressable onPress={() => router.push((message.suggestedAction === "open_request" ? "/assistant/request-intake" : message.suggestedAction === "track_transaction" ? "/(tabs)/transactions" : "/inquiries") as never)} style={styles.nextAction}><Text style={[styles.nextActionText, { writingDirection: direction }]}>{isArabic ? "تنفيذ الخطوة المقترحة" : "Take suggested action"}</Text><Ionicons name={isArabic ? "arrow-back" : "arrow-forward"} size={14} color="#FFFFFF" /></Pressable> : null}</View> : null}
