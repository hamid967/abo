...
export default function AssistantScreen() {
  ...
  const text = isArabic ? { title: "مساعد أبو مشعل", subtitle: "إرشاد مبني على مصادر المعرفة المنشورة", notice: "المساعد معلوماتي فقط، ولا يقدم استشارة قانونية ملزمة أو قراراً حكومياً. عند غياب مصدر موثوق، سيوجهك إلى فريق الدعم.", placeholder: "اكتب سؤالك عن متابعة طلبك أو المتطلبات", send: "إرسال", source: "مصادر المعرفة", updated: "آخر تحديث", support: "فتح تذكرة دعم", empty: "ابدأ سؤالك، وسيرد المساعد بما تغطيه المصادر المعتمدة.", signIn: "سجّل الدخول لاستخدام المساعد الإرشادي.", typing: "المساعد يكتب رداً من المصادر المعتمدة…", suggestions: "أسئلة مقترحة", copy: "نسخ الرد", copied: "تم النسخ", copyFailed: "تعذر النسخ", copiedHistory: "آخر الردود المنسوخة", localOnly: "محفوظة على هذا الجهاز فقط", recopy: "نسخ مجدداً" } : { title: "Abu Mishal Assistant", subtitle: "Guidance grounded in published knowledge sources", notice: "The assistant is informational only. It does not provide binding legal advice or government decisions. If no trusted source exists, it will direct you to support.", placeholder: "Ask about request follow-up or requirements", send: "Send", source: "Knowledge sources", updated: "Updated", support: "Open a support ticket", empty: "Ask a question and the assistant will respond using approved sources.", signIn: "Sign in to use the guidance assistant.", typing: "The assistant is preparing a response from approved sources…", suggestions: "Suggested questions", copy: "Copy response", copied: "Copied", copyFailed: "Copy failed", copiedHistory: "Recently copied responses", localOnly: "Stored on this device only", recopy: "Copy again" };
  const suggestions: AssistantSuggestion[] = isArabic ? [
    { label: "ما حالة معاملتي؟", action: "ask" },
    { label: "ما المستندات الناقصة؟", action: "ask" },
    { label: "أريد الاستفسار عن خدمة.", action: "ask" },
    { label: "أنشئ لي طلباً جديداً.", action: "request" },
    { label: "أريد التواصل مع الموظف.", action: "support" },
  ] : [
    { label: "What is my transaction status?", action: "ask" },
    { label: "Which documents are missing?", action: "ask" },
    { label: "I want to ask about a service.", action: "ask" },
    { label: "Create a new request.", action: "request" },
    { label: "I want to contact the team.", action: "support" },
  ];
  useEffect(() => {
    let mounted = true;
    if (!copiedHistoryKey) {
    ...
    void AsyncStorage.getItem(copiedHistoryKey).then((rawValue) => {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          setCopiedHistory(parsed.filter((entry): entry is CopiedResponse => Boolean(entry && typeof entry.id === "string" && typeof entry.text === "string" && typeof entry.copiedAt === "string")).slice(0, 8));
        }
      } catch {
        setCopiedHistory([]);
      }
  ...
  const handleSuggestion = (suggestion: AssistantSuggestion) => {
    if (suggestion.action === "request") {
      router.push("/request/new" as never);
      return;
    }
    if (suggestion.action === "support") {
      router.push("/inquiries" as never);
      return;
    }
  ...
  const copyAnswer = async (message: ChatMessage) => {
    try {
      ...
      setCopyStatus({ id: message.id, state: "failed" });
    }
