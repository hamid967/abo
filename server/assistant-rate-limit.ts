...
export function checkAssistantRateLimit(input: { userId: number; action: "start" | "message" | "voice" | "document_summary"; now?: number }) {
  const now = input.now ?? Date.now();
  const windowMs = input.action === "message" || input.action === "voice" ? 15 * 60_000 : 60 * 60_000;
  const limit = input.action === "message" ? 30 : input.action === "voice" ? 10 : input.action === "document_summary" ? 8 : 12;
