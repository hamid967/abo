...
import {
  ...
  assertSafeConversationContent,
  conversationStatusForState,
  type ConversationState,
} from "./conversation-state";
import {
  calculateDraftCompletion,
  mergeRequestDraftData,
  type RequestDraftPatch,
} from "./request-draft-policy";
import {
  validateRequestData,
  validationStatusFromResults,
...
import {
  classifyLoginSecurity,
  formatLoginSecurityAlert,
  normalizeDeviceId,
} from "./login-security";
import {
  dueAtForPlaybookStep,
  playbookTaskSourceKey,
  resolveGeneratedTaskAssignee,
  shouldGenerateTaskFromPlaybookStep,
  slaDueAtForPlaybookStep,
  slaMinutesForPlaybookStep,
} from "./playbook-task-policy";
import {
  isOfficialGovernmentUrl,
  mayTransitionRegulatoryUpdate,
  parseOfficialRss,
  type OfficialUpdateImportance,
  type OfficialUpdateType,
  ZATCA_OFFICIAL_SOURCE,
} from "./official-update-policy";
...
export async function createExpoGoOAuthAttempt(input: {
  ...
  await db
    .delete(expoGoOAuthAttempts)
    .where(lt(expoGoOAuthAttempts.expiresAt, new Date()));
...
export async function claimExpoGoOAuthAttempt(input: {
  id: string;
  proofHash: string;
}) {
  const db = await getDb();
...
export async function failExpoGoOAuthAttempt(id: string) {
  ...
  await db
    .update(expoGoOAuthAttempts)
    .set({ status: "failed", authorizationCode: null })
    .where(eq(expoGoOAuthAttempts.id, id));
}
...
export async function createServiceRequest(
  input: Omit<InsertServiceRequest, "requestNumber" | "customerUserId"> & {
  ...
  const result = await db
    ...
    .values({ ...input, requestNumber });
  return { id: result[0].insertId, requestNumber };
}

export async function canUseOrganization(
  userId: number,
...
export async function listDailyDueCandidates(
  before: Date,
): Promise<DailyDueCandidate[]> {
  const db = await getDb();
  if (!db) return [];
  const [requestRows, transactionRows, appointmentRows, taskRows] =
    await Promise.all([
      db
        .select({
          id: serviceRequests.id,
          recipientUserId: serviceRequests.customerUserId,
          title: serviceRequests.title,
          dueAt: serviceRequests.desiredDueAt,
        })
        .from(serviceRequests)
        .where(
          and(
            isNotNull(serviceRequests.desiredDueAt),
            isNull(serviceRequests.deletedAt),
            notInArray(serviceRequests.status, ["cancelled"]) as never,
            lt(serviceRequests.desiredDueAt, before),
          ),
        ),
      db
        .select({
          id: transactions.id,
          recipientUserId: transactions.customerUserId,
        ...
        .where(
          and(
            isNotNull(transactions.dueAt),
            isNull(transactions.deletedAt),
            notInArray(transactions.status, [
              "completed",
              "rejected",
              "cancelled",
              "archived",
            ]) as never,
            lt(transactions.dueAt, before),
          ),
        ),
      db
        .select({
          id: appointments.id,
          recipientUserId: appointments.customerUserId,
          title: appointments.title,
          dueAt: appointments.startsAt,
        })
        .from(appointments)
        .where(
          and(
            eq(appointments.status, "scheduled"),
            lt(appointments.startsAt, before),
          ),
        ),
      db
        .select({
          ...
          title: tasks.title,
          dueAt: sql<Date | null>`coalesce(${tasks.slaDueAt}, ${tasks.dueAt})`,
        })
        .from(tasks)
        .where(
          and(
            or(isNotNull(tasks.slaDueAt), isNotNull(tasks.dueAt)),
  ...
  return [
    ...requestRows
      .filter(
        (row): row is typeof row & { dueAt: Date } => row.dueAt instanceof Date,
      )
      .map((row) => ({
    ...
    ...transactionRows
      ...
      .map((row) => ({
        ...
        resourceId: String(row.id),
        recipientUserId: row.recipientUserId,
        title: row.referenceNumber || `معاملة #${row.id}`,
        dueAt: row.dueAt,
      })),
    ...appointmentRows
      .filter(
    ...
    ...taskRows
      ...
      .flatMap((row) =>
        [
          ...new Set(
            [row.ownerUserId, row.assigneeUserId].filter(
...
export type InactiveDraftCandidate = {
  conversationId: string;
  recipientUserId: number;
  lastActivityAt: Date;
};

export async function listInactiveDraftCandidates(
  before: Date,
): Promise<InactiveDraftCandidate[]> {
  const db = await getDb();
  if (!db) return [];
  const rows = await db
    .select({
      conversationId: aiConversations.id,
      recipientUserId: aiConversations.ownerUserId,
      lastActivityAt: aiConversations.lastActivityAt,
    })
    .from(aiConversations)
    .where(
      and(
        eq(aiConversations.status, "active"),
        notInArray(aiConversations.currentState, [
          "submitted",
          "cancelled",
          "expired",
          "needs_human_review",
        ]),
        lt(aiConversations.lastActivityAt, before),
      ),
    )
    .limit(300);
  return rows.filter(
    (row): row is typeof row & { lastActivityAt: Date } =>
      row.lastActivityAt instanceof Date,
  );
}

type DailyNotificationKey = {
  recipientUserId: number;
  resourceType: string;
  resourceId: string;
...
export async function releaseDailyDueNotification(input: DailyNotificationKey) {
  ...
  if (!db) return;
  await db
    .delete(dueNotificationRuns)
    .where(
      and(
        eq(dueNotificationRuns.recipientUserId, input.recipientUserId),
        eq(dueNotificationRuns.resourceType, input.resourceType),
        eq(dueNotificationRuns.resourceId, input.resourceId),
        eq(dueNotificationRuns.notifiedForDate, input.notifiedForDate),
        isNull(dueNotificationRuns.notificationId),
      ),
    );
}

export async function finalizeDailyDueNotification(
  input: DailyNotificationKey & { notificationId: number },
) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(dueNotificationRuns)
    .set({ notificationId: input.notificationId })
    .where(
      and(
        eq(dueNotificationRuns.recipientUserId, input.recipientUserId),
        eq(dueNotificationRuns.resourceType, input.resourceType),
...
export async function getDailyDueScanSchedule() {
  ...
  const rows = await db
    ...
    .where(eq(automationSchedules.key, "daily_due_scan"))
    .limit(1);
  return rows[0];
}

export async function saveDailyDueScanSchedule(input: {
  ...
  await db
    ...
    .values({ key: "daily_due_scan", ...input })
    .onDuplicateKeyUpdate({
      set: { heartbeatTaskUid: input.heartbeatTaskUid, enabled: input.enabled },
    });
  return getDailyDueScanSchedule();
}

export async function updateDailyDueScanRun(input: {
  success: boolean;
  summary: unknown;
}) {
  const db = await getDb();
...
export async function listServiceRequests(userId: number, role: string) {
  ...
  return canOperateTransactions(role)
    ? query
    : query.where(eq(serviceRequests.customerUserId, userId));
}
...
export async function listTransactions(userId: number, role: string) {
  ...
  return canOperateTransactions(role)
    ? query
    : query.where(eq(transactions.customerUserId, userId));
}
...
function networkFingerprint(req: {
  ip?: string;
...
export async function recordLoginSecurityEvent(input: {
  ...
  const deviceFingerprint = securityHash(
    normalizeDeviceId(
      input.deviceId,
      input.platform,
      String(input.req.headers?.["user-agent"] || "unknown"),
    ),
  );
  const network = networkFingerprint(input.req);
  const knownDevices = await db
    .select({
      id: loginSecurityDevices.id,
      networkFingerprint: loginSecurityDevices.networkFingerprint,
    })
    .from(loginSecurityDevices)
    .where(eq(loginSecurityDevices.userId, input.userId))
    .limit(50);
  const classification = classifyLoginSecurity(
    knownDevices,
    deviceFingerprint,
    network,
  );
  ...
  if (existing) {
    await db
      .update(loginSecurityDevices)
      .set({
        networkFingerprint: network,
        platform: input.platform ?? undefined,
        userAgent: String(input.req.headers?.["user-agent"] || "").slice(
          0,
          512,
        ),
        lastSeenAt: new Date(),
      })
      .where(eq(loginSecurityDevices.id, existing.id));
  } else {
    await db.insert(loginSecurityDevices).values({
      id: deviceFingerprint,
      userId: input.userId,
      deviceFingerprint,
      networkFingerprint: network,
      platform: input.platform ?? undefined,
      userAgent: String(input.req.headers?.["user-agent"] || "").slice(0, 512),
    });
  }
  if (isNewDevice || isUnusualNetwork) {
    const alert = formatLoginSecurityAlert({
      isNewDevice,
      isUnusualNetwork,
...
export async function createSupportTicket(input: {
  ...
  channel?: "support" | "abu_mishal_chat";
}) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function listSupportTickets(userId: number, role: string) {
  ...
  const query = db
    ...
    .from(supportTickets)
    .leftJoin(users, eq(supportTickets.customerUserId, users.id));
  return canOperateTransactions(role)
    ? query
        .where(eq(supportTickets.channel, "support"))
        .orderBy(desc(supportTickets.updatedAt))
        .limit(100)
    : query
        .where(
          and(
            eq(supportTickets.customerUserId, userId),
            eq(supportTickets.channel, "support"),
          ),
        )
        .orderBy(desc(supportTickets.updatedAt))
        .limit(100);
...
export async function addTicketMessage(input: {
  ...
  body: string;
  isInternal: boolean;
  nextStatus: "in_progress" | "awaiting_customer";
}) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getCustomerAbuMishalChat(customerUserId: number) {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(supportTickets.customerUserId, customerUserId),
        eq(supportTickets.channel, "abu_mishal_chat"),
      ),
    )
    .orderBy(desc(supportTickets.updatedAt))
    .limit(1);
...
export async function createCustomerAbuMishalChat(customerUserId: number) {
  ...
  const result = await db.insert(supportTickets).values({
    customerUserId,
    channel: "abu_mishal_chat",
    subject: "محادثة مع أبو مشعل",
    priority: "normal",
    status: "open",
  });
...
export async function listAbuMishalChatThreads() {
  ...
  return db
    ...
    .where(eq(supportTickets.channel, "abu_mishal_chat"))
    .groupBy(
      supportTickets.id,
      supportTickets.customerUserId,
      users.name,
      supportTickets.subject,
      supportTickets.status,
      supportTickets.assignedUserId,
...
export async function markAbuMishalChatRead(input: {
  ticketId: number;
  viewerUserId: number;
  customerUserId: number;
  adminViewer: boolean;
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const authorCondition = input.adminViewer
    ? eq(ticketMessages.authorUserId, input.customerUserId)
    : ne(ticketMessages.authorUserId, input.viewerUserId);
  await db
...
export async function listLoginActivity(userId: number) {
  ...
  const [devices, alerts] = await Promise.all([
    db
      .select({
        id: loginSecurityDevices.id,
        platform: loginSecurityDevices.platform,
        userAgent: loginSecurityDevices.userAgent,
        lastSeenAt: loginSecurityDevices.lastSeenAt,
        createdAt: loginSecurityDevices.createdAt,
      })
      .from(loginSecurityDevices)
      .where(eq(loginSecurityDevices.userId, userId))
      .orderBy(desc(loginSecurityDevices.lastSeenAt))
      .limit(30),
    db
      .select({
        id: auditLogs.id,
      ...
      .where(
        and(
          ...
          eq(auditLogs.action, "auth.login_security_alert"),
        ),
      )
...
export async function forgetLoginSecurityDevice(
  userId: number,
  deviceId: string,
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db
    .delete(loginSecurityDevices)
    .where(
      and(
        eq(loginSecurityDevices.id, deviceId),
        eq(loginSecurityDevices.userId, userId),
      ),
    );
  return {
    success: true,
    removed: Number(result[0]?.affectedRows ?? 0) > 0,
  } as const;
}
...
export async function listNotificationCenter(userId: number) {
  ...
  const deliveryRows = await db
    .select({
      notificationId: notificationDeliveryLogs.notificationId,
    ...
    .where(inArray(notificationDeliveryLogs.notificationId, notificationIds))
    .orderBy(desc(notificationDeliveryLogs.createdAt))
    .limit(240);
  const taskIds = [
    ...new Set(
      items
        .map((item) => notificationTaskId(item.data))
  ...
  return items.map((item) => ({
    ...item,
    task: notificationTaskId(item.data)
      ? (tasksById.get(notificationTaskId(item.data)!) ?? null)
      : null,
...
export async function createInAppNotification(input: {
  ...
  const critical =
    input.type === "support_ticket" || input.type === "human_handoff";
  const status = !preferences.inAppEnabled
    ? "suppressed"
    : preferences.digestFrequency === "daily" && !critical
      ? "queued"
      : "delivered";
  await db.insert(notificationDeliveryLogs).values({
    ...
    notificationId,
    status,
    idempotencyKey: `in-app:${notificationId}`,
    details: {
...
export async function updateNotificationPreferences(input: {
  ...
  const taskReminderMinutes =
    input.taskReminderMinutes ?? current.taskReminderMinutes;
  const pushEnabled = input.pushEnabled ?? current.pushEnabled;
  const taskAlertsEnabled =
    input.taskAlertsEnabled ?? current.taskAlertsEnabled;
  const calendarSyncEnabled =
    input.calendarSyncEnabled ?? current.calendarSyncEnabled;
  const quietStartHour = input.quietHoursEnabled
    ? (input.quietStartHour ?? 22)
    : null;
  const quietEndHour = input.quietHoursEnabled
    ? (input.quietEndHour ?? 7)
    : null;
  await db
...
export async function registerMobilePushDevice(input: {
  ...
  await db
    ...
    .where(
      and(
        eq(mobilePushDevices.expoPushToken, input.expoPushToken),
        ne(mobilePushDevices.userId, input.userId),
      ),
    );
  const existing = await db
    .select({ id: mobilePushDevices.id })
  ...
  if (existing[0]) {
    await db
      ...
      .where(eq(mobilePushDevices.id, existing[0].id));
    return { id: existing[0].id, reused: true } as const;
  }
  const id = crypto.randomUUID();
  await db
    .insert(mobilePushDevices)
    .values({ id, ...input, enabled: true, lastSeenAt: new Date() });
  return { id, reused: false } as const;
}

export async function deactivateMobilePushDevice(input: {
  userId: number;
  deviceId: string;
}) {
  const db = await getDb();
...
export async function listMobilePushDevicesForUser(userId: number) {
  ...
  return db
    ...
    .from(mobilePushDevices)
    .where(eq(mobilePushDevices.userId, userId))
...
export async function listActivePushTokensForUser(userId: number) {
  ...
  return db
    .select({
      id: mobilePushDevices.id,
      expoPushToken: mobilePushDevices.expoPushToken,
      platform: mobilePushDevices.platform,
    })
    .from(mobilePushDevices)
    .where(
      and(
        eq(mobilePushDevices.userId, userId),
...
export async function createPushDeliveryLog(input: {
  notificationId: number;
  status: "queued" | "delivered" | "suppressed" | "failed";
  ...
  try {
    await db
      ...
      .values({ id: crypto.randomUUID(), ...input, channel: "push" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "";
    if (message.includes("Duplicate") || message.includes("ER_DUP_ENTRY"))
      return;
...
export async function listNotificationDeliveryLogs(userId: number) {
  ...
  return db
    ...
    .innerJoin(
      notifications,
      eq(notificationDeliveryLogs.notificationId, notifications.id),
    )
    .where(eq(notifications.recipientUserId, userId))
    .orderBy(desc(notificationDeliveryLogs.createdAt))
    .limit(60);
...
export async function listPublishedKnowledge(language: "ar" | "en") {
  ...
  const [articles, faqs] = await Promise.all([
    ...
    db
      ...
      .from(faqItems)
      .where(
        and(eq(faqItems.isPublished, true), eq(faqItems.language, language)),
      )
...
export async function getKnowledgeContext(language: "ar" | "en" = "ar") {
  ...
  const sources = [
    ...articles.map((article) => ({
      title: article.title,
      sourceLabel: article.sourceLabel ?? "مركز معرفة أبو مشعل",
      sourceUrl: article.sourceUrl ?? null,
      updatedAt: article.updatedAt,
    })),
    ...faqs.map((faq) => ({
      title: faq.question,
      sourceLabel: "الأسئلة الشائعة في أبو مشعل",
      sourceUrl: null,
...
export async function searchAccessibleRecords(
  ...
  const pattern = `%${term}%`;
  const requestPredicate = or(
    like(serviceRequests.requestNumber, pattern),
    like(serviceRequests.title, pattern),
    like(serviceRequests.customerPhone, pattern),
  );
  const transactionPredicate = or(
    like(transactions.referenceNumber, pattern),
    like(transactions.nextAction, pattern),
  );
  const requestsResult = canOperateTransactions(role)
    ? await db
        .select({
          id: serviceRequests.id,
    ...
    : await db
        ...
        .from(serviceRequests)
        .where(
          and(eq(serviceRequests.customerUserId, userId), requestPredicate),
        )
        .limit(10);
  const transactionsResult = canOperateTransactions(role)
    ? await db
        .select({
          id: transactions.id,
          number: transactions.referenceNumber,
          status: transactions.status,
          nextAction: transactions.nextAction,
        })
        .from(transactions)
    ...
    : await db
        ...
        .from(transactions)
        .where(
          and(eq(transactions.customerUserId, userId), transactionPredicate),
        )
        .limit(10);
...
export async function getCloudRecord(
  ownerUserId: number,
  recordType: "transactions" | "workspace" | "inquiries" | "today-actions",
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const rows = await db
    ...
    .where(
      and(
        eq(cloudRecords.ownerUserId, ownerUserId),
        eq(cloudRecords.recordType, recordType),
      ),
    )
    .limit(1);
  return rows[0];
...
export async function upsertCloudRecord(
  ownerUserId: number,
  recordType: "transactions" | "workspace" | "inquiries" | "today-actions",
  payload: unknown,
) {
  ...
  await db
    .insert(cloudRecords)
    .values({ ownerUserId, recordType, payload })
...
export async function createUploadedDocument(input: {
  ...
  const result = await db
    .insert(documents)
    .values({ ...input, verificationStatus: "pending" });
  return result[0].insertId;
}
...
export async function listPlaybooksForAdmin() {
  ...
  const rows = await db
    .select({
      playbook: servicePlaybooks,
      serviceName: governmentServices.name,
      versionId: playbookVersions.id,
      versionNumber: playbookVersions.versionNumber,
      versionStatus: playbookVersions.status,
      title: playbookVersions.title,
      publishedAt: playbookVersions.publishedAt,
    })
    .from(servicePlaybooks)
    .innerJoin(
      governmentServices,
      eq(servicePlaybooks.serviceId, governmentServices.id),
    )
    .leftJoin(
      playbookVersions,
      eq(playbookVersions.playbookId, servicePlaybooks.id),
    )
    .orderBy(
      desc(servicePlaybooks.updatedAt),
      desc(playbookVersions.versionNumber),
    );
  ...
  for (const row of rows) {
    if (!grouped.has(row.playbook.id))
      grouped.set(row.playbook.id, {
        id: row.playbook.id,
        serviceId: row.playbook.serviceId,
        serviceName: row.serviceName,
        name: row.playbook.name,
        status: row.playbook.status,
        versions: [],
      });
    if (row.versionId)
      grouped.get(row.playbook.id)!.versions.push({
        id: row.versionId,
...
export async function listActiveServicesForPlaybooks() {
  ...
  if (!db) return [];
  return db
    .select({ id: governmentServices.id, name: governmentServices.name })
    .from(governmentServices)
    .where(eq(governmentServices.isActive, true))
    .orderBy(asc(governmentServices.name));
}

export function normalizeServiceSearch(value: string) {
  return value
    .normalize("NFKD")
    .replace(/[\u064B-\u065F\u0670]/g, "")
    .replace(/[إأآ]/g, "ا")
...
export function serviceMatchScore(queryValue: string, candidateValue: string) {
  const query = normalizeServiceSearch(queryValue);
  const candidate = normalizeServiceSearch(candidateValue);
...
/** Returns a conservative match only; uncertain names remain unassigned for human review. */
export async function findMatchingActiveService(serviceName: string) {
  const db = await getDb();
  if (!db) return undefined;
  const query = normalizeServiceSearch(serviceName);
  if (query.length < 3) return undefined;
  const services = await db
    .select({
      id: governmentServices.id,
      name: governmentServices.name,
      description: governmentServices.description,
      requirements: governmentServices.requirements,
    })
    .from(governmentServices)
    .where(eq(governmentServices.isActive, true))
    .orderBy(asc(governmentServices.name))
  ...
  const ranked = services
    .map((service) => {
      return { service, score: serviceMatchScore(query, service.name) };
    })
    .filter((entry) => entry.score >= 0.67)
    .sort((a, b) => b.score - a.score);
  if (!ranked[0] || (ranked[1] && ranked[0].score === ranked[1].score))
    return undefined;
  return { ...ranked[0].service, confidence: ranked[0].score };
}

export async function searchActiveServices(term: string, limit = 6) {
  const db = await getDb();
  if (!db) return [];
  const query = normalizeServiceSearch(term);
  if (query.length < 2) return [];
  const services = await db
    .select({
      id: governmentServices.id,
      name: governmentServices.name,
      audience: governmentServices.audience,
      description: governmentServices.description,
      requirements: governmentServices.requirements,
    })
    .from(governmentServices)
  ...
  return services
    .map((service) => ({
      ...service,
      confidence: serviceMatchScore(query, service.name),
    }))
    .filter((service) => service.confidence >= 0.34)
    .sort(
      (left, right) =>
...
export async function getPublishedPlaybookForService(serviceId: number) {
  ...
  const rows = await db
    .select({
      playbookId: servicePlaybooks.id,
      playbookName: servicePlaybooks.name,
      serviceName: governmentServices.name,
      versionId: playbookVersions.id,
      versionNumber: playbookVersions.versionNumber,
      title: playbookVersions.title,
      description: playbookVersions.description,
      requirements: playbookVersions.requirements,
      exceptions: playbookVersions.exceptions,
    })
    .from(servicePlaybooks)
    .innerJoin(
      governmentServices,
    ...
    .where(
      and(
        eq(servicePlaybooks.serviceId, serviceId),
        eq(servicePlaybooks.status, "active"),
        eq(playbookVersions.status, "published"),
      ),
    )
    .orderBy(desc(playbookVersions.publishedAt))
    .limit(1);
  const active = rows[0];
  if (!active) return undefined;
  const steps = await db
    .select({
      stepKey: playbookSteps.stepKey,
      title: playbookSteps.title,
      instructions: playbookSteps.instructions,
      actionType: playbookSteps.actionType,
      assignmentRule: playbookSteps.assignmentRule,
      slaMinutes: playbookSteps.slaMinutes,
      isRequired: playbookSteps.isRequired,
      expectedDurationMinutes: playbookSteps.expectedDurationMinutes,
      stepOrder: playbookSteps.stepOrder,
    })
    .from(playbookSteps)
    .where(eq(playbookSteps.versionId, active.versionId))
    .orderBy(asc(playbookSteps.stepOrder));
  return { ...active, steps };
}

export async function getPlaybookVersionDetails(
  playbookId: string,
  versionId: string,
) {
  const db = await getDb();
  ...
  const versions = await db
    ...
    .where(
      and(
        ...
        eq(playbookVersions.playbookId, playbookId),
      ),
    )
    .limit(1);
  ...
  const steps = await db
    .select()
    .from(playbookSteps)
    .where(eq(playbookSteps.versionId, versionId))
    .orderBy(asc(playbookSteps.stepOrder));
  return { ...result, steps };
}

export async function createServicePlaybook(input: {
  serviceId: number;
  name: string;
  createdByUserId: number;
}) {
  ...
  const service = await db
    .select({ id: governmentServices.id })
    .from(governmentServices)
    .where(
      and(
        eq(governmentServices.id, input.serviceId),
        eq(governmentServices.isActive, true),
      ),
    )
    .limit(1);
  if (!service[0]) throw new Error("SERVICE_NOT_FOUND");
  const existing = await db
    .select({ id: servicePlaybooks.id })
    .from(servicePlaybooks)
    .where(
      and(
        eq(servicePlaybooks.serviceId, input.serviceId),
        eq(servicePlaybooks.status, "active"),
      ),
    )
    .limit(1);
...
export async function createPlaybookVersion(input: {
  playbookId: string;
  title: string;
  description?: string;
  ...
  const playbook = await db
    ...
    .where(
      and(
        eq(servicePlaybooks.id, input.playbookId),
        eq(servicePlaybooks.status, "active"),
      ),
    )
    .limit(1);
  ...
  const previous = await db
    .select({ versionNumber: playbookVersions.versionNumber })
    .from(playbookVersions)
    ...
    if (input.steps.length)
      await tx.insert(playbookSteps).values(
        input.steps.map((step, index) => ({
          ...
          assignmentRule: step.assignmentRule ?? "transaction_assignee",
...
export async function publishPlaybookVersion(input: {
  ...
  const version = await db
    .select({ id: playbookVersions.id, status: playbookVersions.status })
    .from(playbookVersions)
    .where(
      and(
        eq(playbookVersions.id, input.versionId),
        eq(playbookVersions.playbookId, input.playbookId),
      ),
    )
    .limit(1);
  if (!version[0]) throw new Error("PLAYBOOK_VERSION_NOT_FOUND");
  if (version[0].status === "archived")
    throw new Error("ARCHIVED_VERSION_CANNOT_BE_PUBLISHED");
  const steps = await db
    .select({ id: playbookSteps.id })
    .from(playbookSteps)
    .where(eq(playbookSteps.versionId, input.versionId))
    .limit(1);
    ...
    await tx
      ...
      .where(
        and(
          eq(playbookVersions.playbookId, input.playbookId),
          eq(playbookVersions.status, "published"),
          ne(playbookVersions.id, input.versionId),
        ),
      );
    await tx
      .update(playbookVersions)
...
export async function archiveServicePlaybook(playbookId: string) {
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(servicePlaybooks)
      .set({ status: "archived" })
      .where(
        and(
          eq(servicePlaybooks.id, playbookId),
          eq(servicePlaybooks.status, "active"),
        ),
      );
    await tx
      ...
      .where(
        and(
          eq(playbookVersions.playbookId, playbookId),
          ne(playbookVersions.status, "archived"),
        ),
      );
  });
  return { success: true } as const;
...
export async function listOwnedDocuments(ownerUserId: number) {
  ...
  return db
    ...
    .from(documents)
    .where(
      and(eq(documents.ownerUserId, ownerUserId), isNull(documents.deletedAt)),
    )
    .orderBy(desc(documents.createdAt));
}

export async function getOwnedDocumentForAccess(
  ownerUserId: number,
  documentId: number,
) {
  const db = await getDb();
...
export async function createDocumentFieldExtraction(input: {
  ownerUserId: number;
  documentId: number;
  ...
  if (!db) throw new Error("Database not available");
  const id = randomUUID();
  await db.insert(documentFieldExtractions).values({
    id,
...
export async function confirmDocumentFieldExtraction(input: {
  ownerUserId: number;
  extractionId: string;
}) {
  const db = await getDb();
  ...
  const rows = await db
    .select()
    .from(documentFieldExtractions)
    .where(
      and(
        eq(documentFieldExtractions.id, input.extractionId),
        eq(documentFieldExtractions.ownerUserId, input.ownerUserId),
        eq(documentFieldExtractions.status, "preview"),
      ),
    )
    .limit(1);
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(documentFieldExtractions)
      .set({ status: "confirmed", confirmedAt: new Date() })
      .where(eq(documentFieldExtractions.id, input.extractionId));
    if (extraction.documentType)
      await tx
        .update(documents)
        .set({ documentType: extraction.documentType })
        .where(
          and(
            eq(documents.id, extraction.documentId),
            eq(documents.ownerUserId, input.ownerUserId),
            isNull(documents.deletedAt),
          ),
        );
  });
...
export async function softDeleteOwnedDocument(
  ownerUserId: number,
  documentId: number,
) {
  const db = await getDb();
  ...
  if (document.requestId || document.transactionId)
    throw new Error("DOCUMENT_LINKED_TO_RECORD");
  await db
    .update(documents)
...
export type ApprovalAlertWindowHours =
  (typeof approvalAlertWindowHours)[number];

export async function getAdminApprovalAlertSettings() {
  const db = await getDb();
  const fallback = {
    approvalAlertWindowHours: 24 as ApprovalAlertWindowHours,
  ...
  if (existing[0])
    return {
      approvalAlertWindowHours: existing[0]
        .approvalAlertWindowHours as ApprovalAlertWindowHours,
  ...
  await db
    ...
    .values({
      id: 1,
      approvalAlertWindowHours: fallback.approvalAlertWindowHours,
    })
  ...
  return created[0]
    ? {
        approvalAlertWindowHours: created[0]
          .approvalAlertWindowHours as ApprovalAlertWindowHours,
...
export async function updateAdminApprovalAlertSettings(input: {
  approvalAlertWindowHours: ApprovalAlertWindowHours;
  ...
  await db
    ...
    .values({
      id: 1,
      approvalAlertWindowHours: input.approvalAlertWindowHours,
...
export type MobileAppReleasePlatform =
  | "android_apk"
  | "android_aab"
  | "ios_ipa";
...
export async function listMobileAppReleases() {
  ...
  return db
    ...
    .from(mobileAppReleases)
    .leftJoin(users, eq(mobileAppReleases.createdByUserId, users.id))
    .orderBy(desc(mobileAppReleases.updatedAt));
}
...
export async function saveMobileAppRelease(input: {
  ...
  if (input.id) {
    const result = await db
      .update(mobileAppReleases)
      .set(values)
      .where(eq(mobileAppReleases.id, input.id));
    if (
      Number(
        (result[0] as { affectedRows?: number } | undefined)?.affectedRows ?? 0,
      ) === 0
    )
      throw new Error("MOBILE_RELEASE_NOT_FOUND");
    return input.id;
  }
  const result = await db
    .insert(mobileAppReleases)
    .values({ ...values, createdByUserId: input.createdByUserId });
  return Number(
    (result[0] as { insertId?: number } | undefined)?.insertId ?? 0,
  );
}

export async function getSystemTransactionDashboard(
  ...
  if (!db)
    return {
      metrics: {
        ...
        awaitingDocuments: 0,
        completed: 0,
        approvalsExpiringSoon: 0,
        approvalAlertWindowHours:
          approvalAlertSettings.approvalAlertWindowHours,
      },
      transactions: [],
    };
  const now = new Date();
  const approvalWindowEndsAt = new Date(
    now.getTime() +
      approvalAlertSettings.approvalAlertWindowHours * 60 * 60 * 1000,
  );
  const [grouped, expiringApprovals] = await Promise.all([
    db
      .select({ status: transactions.status, total: count(transactions.id) })
      .from(transactions)
      .groupBy(transactions.status),
    db
      .select({ total: count(approvalRequests.id) })
  ...
  const statusTotals = Object.fromEntries(
    grouped.map((item) => [item.status, Number(item.total)]),
  );
  const total = Object.values(statusTotals).reduce(
    (sum, value) => sum + value,
    0,
  );
  ...
  const inactive =
    ...
    (statusTotals.cancelled ?? 0) +
    (statusTotals.archived ?? 0);
  const criteria = [
    ...
    search
      ? or(
          ...
          like(serviceRequests.customerPhone, `%${search}%`),
  ...
  const rowQuery = db
    .select({
      ...
      assigneeUserId: transactions.assigneeUserId,
    })
    .from(transactions)
    .leftJoin(users, eq(transactions.customerUserId, users.id))
    .leftJoin(serviceRequests, eq(transactions.requestId, serviceRequests.id))
  ...
  const rows = criteria.length
    ? await rowQuery
        ...
        .orderBy(desc(transactions.updatedAt))
        .limit(100)
    : await rowQuery.orderBy(desc(transactions.updatedAt)).limit(100);
  return {
    metrics: {
      total,
      active: total - inactive,
      overdue: statusTotals.overdue ?? 0,
      awaitingDocuments: statusTotals.awaiting_customer_documents ?? 0,
      completed,
      approvalsExpiringSoon: Number(expiringApprovals[0]?.total ?? 0),
      approvalAlertWindowHours: approvalAlertSettings.approvalAlertWindowHours,
    },
    transactions: rows,
  };
}
...
export async function getTaskWorkloadOverview() {
  ...
  const members = assigneeIds.length
    ...
    : [];
  const names = new Map(
    members.map((member) => [member.id, member.name || `عضو #${member.id}`]),
  );
  const team = grouped
    .map((row) => ({
      assigneeUserId: row.assigneeUserId,
      name: row.assigneeUserId
        ? (names.get(row.assigneeUserId) ?? `عضو #${row.assigneeUserId}`)
        : "غير معيّنة",
  ...
  return {
    totalActive: team.reduce((sum, row) => sum + row.active, 0),
    overdue: team.reduce((sum, row) => sum + row.overdue, 0),
    unassigned: team.find((row) => row.assigneeUserId === null)?.active ?? 0,
    team,
  };
}
...
export async function createRequestConversation(input: {
  ownerUserId: number;
  language: "ar" | "en";
  idempotencyKey: string;
}) {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(requestDrafts.ownerUserId, input.ownerUserId),
        eq(requestDrafts.idempotencyKey, input.idempotencyKey),
      ),
    )
    .limit(1);
  if (existing[0]) {
    const conversations = await db
      ...
      .where(
        and(
          eq(aiConversations.ownerUserId, input.ownerUserId),
          eq(aiConversations.draftId, existing[0].id),
        ),
      )
  ...
  await db.transaction(async (tx) => {
    await tx.insert(requestDrafts).values({
      id: draftId,
      ownerUserId: input.ownerUserId,
      structuredData: {},
  ...
  const [draft, conversation] = await Promise.all([
    db
      .select()
      .from(requestDrafts)
      .where(eq(requestDrafts.id, draftId))
      .limit(1),
    db
      .select()
      .from(aiConversations)
...
export async function getRequestConversation(
  ...
  if (!db) throw new Error("Database not available");
  const rows = await db
    .select({ conversation: aiConversations, draft: requestDrafts })
    .from(aiConversations)
    .leftJoin(requestDrafts, eq(aiConversations.draftId, requestDrafts.id))
    .where(
      and(
...
export async function appendConversationMessage(input: {
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const conversation = await getRequestConversation(
    input.ownerUserId,
...
export async function saveConversationProgress(input: {
  ownerUserId: number;
  conversationId: string;
  nextState: ConversationState;
  structuredData?: DraftStructuredData;
  validationStatus?: "pending" | "errors" | "warnings" | "passed";
  completionPercentage?: number;
}) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const current = await getRequestConversation(
    input.ownerUserId,
    input.conversationId,
  );
  const draft = current?.draft;
  if (!current?.conversation || !draft)
    throw new Error("CONVERSATION_NOT_FOUND");
  assertConversationTransition(
    current.conversation.currentState as ConversationState,
    input.nextState,
  );
  const completionPercentage =
    input.completionPercentage === undefined
      ? undefined
      : Math.max(0, Math.min(100, Math.round(input.completionPercentage)));
  await db.transaction(async (tx) => {
    if (
    ...
    await tx
      ...
      .set({
        ...
        status: conversationStatusForState(input.nextState),
...
export async function updateRequestDraftFields(input: {
  ownerUserId: number;
  conversationId: string;
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const session = await getRequestConversation(
    input.ownerUserId,
    input.conversationId,
  );
  ...
  if (
    ...
    !draft ||
    draft.deletedAt ||
    ["submitted", "cancelled", "expired"].includes(draft.status)
  )
    throw new Error("DRAFT_NOT_EDITABLE");
  const structuredData = mergeRequestDraftData(
...
export async function listActiveRequestDrafts(ownerUserId: number) {
  ...
  return db
    .select({
      id: requestDrafts.id,
      status: requestDrafts.status,
      completionPercentage: requestDrafts.completionPercentage,
      structuredData: requestDrafts.structuredData,
      updatedAt: requestDrafts.updatedAt,
      conversationId: aiConversations.id,
    })
    .from(requestDrafts)
    .innerJoin(aiConversations, eq(aiConversations.draftId, requestDrafts.id))
    .where(
      and(
        eq(requestDrafts.ownerUserId, ownerUserId),
        eq(aiConversations.ownerUserId, ownerUserId),
        isNull(requestDrafts.deletedAt),
        notInArray(requestDrafts.status, [
          "submitted",
          "cancelled",
          "expired",
        ]) as never,
...
export async function cancelRequestConversation(
  ownerUserId: number,
  conversationId: string,
) {
  const db = await getDb();
  ...
  if (!session?.conversation || !session.draft)
    throw new Error("CONVERSATION_NOT_FOUND");
  if (
    session.draft.submittedRequestId ||
    session.conversation.status === "submitted"
  )
    throw new Error("SUBMITTED_REQUEST_CANNOT_BE_CANCELLED_HERE");
  if (session.conversation.status === "cancelled")
    return { success: true, alreadyCancelled: true } as const;
  await db.transaction(async (tx) => {
    await tx
      .update(requestDrafts)
      ...
      .where(
        and(
          eq(requestDrafts.id, session.draft!.id),
          eq(requestDrafts.ownerUserId, ownerUserId),
        ),
      );
    await tx
...
export async function deleteAssistantConversationContent(
  ownerUserId: number,
  conversationId: string,
) {
  const db = await getDb();
  ...
  await db.transaction(async (tx) => {
    ...
    if (session.draft && !session.draft.submittedRequestId)
      await tx
        .update(requestDrafts)
    ...
    if (session.conversation.status !== "submitted")
      await tx
        .update(aiConversations)
        .set({
          status: "cancelled",
  ...
  return {
    success: true,
    submittedRequestPreserved: Boolean(session.draft?.submittedRequestId),
  } as const;
}

export async function validateRequestDraft(
  ownerUserId: number,
  conversationId: string,
) {
  const db = await getDb();
  ...
  const session = await getRequestConversation(ownerUserId, conversationId);
  const draft = session?.draft;
  if (!session?.conversation || !draft || draft.deletedAt)
    throw new Error("DRAFT_NOT_FOUND");
  const data =
    draft.structuredData &&
    typeof draft.structuredData === "object" &&
    !Array.isArray(draft.structuredData)
      ? (draft.structuredData as Record<string, unknown>)
      : {};
  ...
  const duplicates = title
    ? await db
        ...
        .where(
          and(
            eq(serviceRequests.customerUserId, ownerUserId),
            eq(serviceRequests.title, title),
            notInArray(serviceRequests.status, ["submitted", "cancelled"]),
          ),
        )
        .limit(1)
    : [];
  ...
  await db
    ...
    .set({
      validationStatus,
      status: validationStatus === "errors" ? "draft" : "reviewing",
    })
    .where(
      and(
        eq(requestDrafts.id, draft.id),
...
export async function moveConversationState(
  ownerUserId: number,
  conversationId: string,
  nextState: ConversationState,
) {
...
export async function prepareDraftReview(
  ownerUserId: number,
  conversationId: string,
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const beforeValidation = await getRequestConversation(
    ownerUserId,
    conversationId,
  );
  if (
    !beforeValidation?.conversation ||
    !beforeValidation.draft ||
    beforeValidation.draft.deletedAt
  )
    throw new Error("DRAFT_NOT_FOUND");
  if (beforeValidation.conversation.currentState === "awaiting_confirmation")
  ...
  if (!reviewableStates.includes(beforeValidation.conversation.currentState))
    throw new Error("REVIEW_STATE_REQUIRED");
  const validation = await validateRequestDraft(ownerUserId, conversationId);
  if (validation.validationStatus === "errors")
...
export async function recordDraftConsent(input: {
  ownerUserId: number;
  conversationId: string;
  consentType: "terms" | "privacy" | "request_submission";
}) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const session = await getRequestConversation(
  ...
  if (
    !session?.conversation ||
    !session.draft ||
    session.conversation.currentState !== "awaiting_confirmation"
  )
...
export async function checkDraftSubmissionConsent(
  ownerUserId: number,
  conversationId: string,
) {
  const db = await getDb();
  ...
  const rows = await db
    .select({ consentType: userConsents.consentType })
    .from(userConsents)
    .where(
      and(
        eq(userConsents.ownerUserId, ownerUserId),
        eq(userConsents.draftId, session.draft.id),
        eq(userConsents.summaryVersion, session.draft.summaryVersion),
        isNull(userConsents.revokedAt),
      ),
    );
  return {
    allowed: mayConfirmDraft({
      state: session.conversation.currentState,
      validationStatus: session.draft.validationStatus,
      consentTypes: rows.map((row) => row.consentType),
      summaryVersion: session.draft.summaryVersion,
    }),
    session,
  };
}
...
export async function submitRequestDraft(
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const consent = await checkDraftSubmissionConsent(
    ownerUserId,
    conversationId,
  );
  const draft = consent.session?.draft;
  if (!consent.session?.conversation || !draft)
    throw new Error("DRAFT_NOT_FOUND");
  if (draft.submittedRequestId) {
    const existingTransaction = await db
      .select({ id: transactions.id })
      .from(transactions)
      .where(eq(transactions.requestId, draft.submittedRequestId))
      .limit(1);
    return {
      requestId: draft.submittedRequestId,
  ...
  const beneficiaryType = data.beneficiaryType as
    | "individual"
    | "establishment"
    | "company"
    | "association"
  ...
  const claimed = await db
    ...
    .where(
      and(
        eq(requestDrafts.id, draft.id),
        eq(requestDrafts.ownerUserId, ownerUserId),
        eq(requestDrafts.status, "awaiting_confirmation"),
      ),
    );
  if (!claimed[0]?.affectedRows) {
    const latest = await getRequestConversation(ownerUserId, conversationId);
    if (latest?.draft?.submittedRequestId)
      return {
        requestId: latest.draft.submittedRequestId,
        transactionId: null,
        alreadySubmitted: true,
      };
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const transactionResult = await tx.insert(transactions).values({
        ...
        status: "received",
        priority,
        nextAction: submissionMessage(language),
      });
      ...
      if (draft.serviceId) {
        const published = await tx
          .select({
            ...
            versionId: playbookVersions.id,
            versionNumber: playbookVersions.versionNumber,
            versionTitle: playbookVersions.title,
            requirements: playbookVersions.requirements,
            exceptions: playbookVersions.exceptions,
          })
          .from(servicePlaybooks)
          ...
          .where(
            and(
              eq(servicePlaybooks.serviceId, draft.serviceId),
              eq(servicePlaybooks.status, "active"),
              eq(playbookVersions.status, "published"),
            ),
          )
        ...
        if (active) {
          const steps = await tx
            ...
            .where(eq(playbookSteps.versionId, active.versionId))
            .orderBy(asc(playbookSteps.stepOrder));
          await tx.insert(requestPlaybookAssignments).values({
            id: crypto.randomUUID(),
            requestId,
            ...
            snapshot: {
              playbookName: active.playbookName,
              versionNumber: active.versionNumber,
              versionTitle: active.versionTitle,
          ...
          const staff = await tx
            .select({ id: users.id })
            .from(users)
            .where(inArray(users.role, ["employee", "supervisor"]));
          const staffIds = staff.map((member) => member.id);
          const loadRows = staffIds.length
            ? await tx
                .select({
                  assigneeUserId: tasks.assigneeUserId,
                ...
                .where(
                  and(
                    inArray(tasks.assigneeUserId, staffIds),
          ...
          const openLoads = new Map(
            loadRows
              .filter(
                (row): row is typeof row & { assigneeUserId: number } =>
                  row.assigneeUserId !== null,
              )
              .map((row) => [row.assigneeUserId, Number(row.openCount)]),
          );
          const leastLoadedStaff = () =>
            staffIds
              .slice()
              .sort(
                (left, right) =>
                  (openLoads.get(left) ?? 0) - (openLoads.get(right) ?? 0) ||
                  left - right,
              )[0];
          const generatedTasks = steps
            .filter(shouldGenerateTaskFromPlaybookStep)
            .map((step) => {
              const assignee = resolveGeneratedTaskAssignee({
                rule: step.assignmentRule,
                requestOwnerUserId: ownerUserId,
                leastLoadedStaffUserId: leastLoadedStaff(),
              });
              if (
                assignee.assigneeUserId &&
                assignee.assignmentSource === "least_loaded_staff"
              )
                openLoads.set(
                  assignee.assigneeUserId,
                  (openLoads.get(assignee.assigneeUserId) ?? 0) + 1,
                );
              ...
              return {
                ...
                assignmentSource: assignee.assignmentSource,
                sourceType: "playbook_step" as const,
                sourceKey: playbookTaskSourceKey(
                  active.versionId,
                  step.stepKey,
                ),
                title: step.title,
                description:
                  step.instructions ??
                  (language === "ar"
                    ? `إجراء مولّد من Playbook ${active.playbookName} (الإصدار ${active.versionNumber}).`
                    : `Task generated from ${active.playbookName} (version ${active.versionNumber}).`),
                priority,
                dueAt: dueAtForPlaybookStep(step),
                slaMinutes,
                slaDueAt: slaDueAtForPlaybookStep(step, priority),
              };
            });
      ...
      const linkedDocuments = await tx
        .select({ documentId: requestDraftDocuments.documentId })
        .from(requestDraftDocuments)
        .where(eq(requestDraftDocuments.draftId, draft.id));
      if (linkedDocuments.length)
        await tx
          .update(documents)
          .set({ requestId, transactionId })
          .where(
            and(
              eq(documents.ownerUserId, ownerUserId),
              inArray(
                documents.id,
                linkedDocuments.map((item) => item.documentId),
              ),
            ),
          );
      await tx.insert(transactionStatusHistory).values({
        ...
        customerNote: submissionMessage(language),
      });
      await tx.insert(tasks).values({
        transactionId,
        ownerUserId,
        title:
          language === "ar" ? "مراجعة الطلب الجديد" : "Review the new request",
        description:
          language === "ar"
            ? "راجع البيانات والمستندات قبل متابعة الإجراء التالي."
            : "Review the data and documents before the next action.",
        priority,
        status: "new",
      });
      await tx
        .update(requestDrafts)
        .set({ status: "submitted", submittedRequestId: requestId })
        .where(
          and(
            eq(requestDrafts.id, draft.id),
            eq(requestDrafts.ownerUserId, ownerUserId),
      ...
      await tx.insert(notifications).values({
        ...
        title: language === "ar" ? "تم إنشاء طلبك" : "Your request was created",
        body: submissionMessage(language),
        type: "request_created",
        data: { requestId, transactionId, requestNumber },
      });
      await tx.insert(auditLogs).values({
        actorUserId: ownerUserId,
        action: "assistant.request_submitted",
        resourceType: "request_draft",
        resourceId: draft.id,
        metadata: {
          requestId,
          transactionId,
    ...
    await db
      ...
      .where(
        and(
          eq(requestDrafts.id, draft.id),
          eq(requestDrafts.ownerUserId, ownerUserId),
          eq(requestDrafts.status, "submitting"),
        ),
      );
    throw error;
  }
...
export async function attachDocumentToDraft(input: {
  ownerUserId: number;
  conversationId: string;
  documentId: number;
  ...
  const draft = session?.draft;
  if (!draft || ["submitted", "cancelled", "expired"].includes(draft.status))
    throw new Error("DRAFT_NOT_EDITABLE");
  const rows = await db
    .select()
    .from(documents)
  ...
  if (!document) throw new Error("DOCUMENT_NOT_FOUND");
  const check = canAttachDraftDocument({
    mimeType: document.mimeType,
    fileSizeBytes: document.fileSizeBytes,
    ownerMatches:
      document.ownerUserId === input.ownerUserId && !document.deletedAt,
  });
  if (!check.allowed) throw new Error(check.reason);
  await db
    .insert(requestDraftDocuments)
    .values({
      id: crypto.randomUUID(),
      draftId: draft.id,
...
export async function listDraftDocuments(
  ...
  return db
    .select({
      id: requestDraftDocuments.id,
      requirementKey: requestDraftDocuments.requirementKey,
      classificationStatus: requestDraftDocuments.classificationStatus,
    ...
    .from(requestDraftDocuments)
    .innerJoin(documents, eq(requestDraftDocuments.documentId, documents.id))
    .where(
      and(
        eq(requestDraftDocuments.draftId, session.draft.id),
        eq(documents.ownerUserId, ownerUserId),
        isNull(documents.deletedAt),
      ),
    )
    .orderBy(desc(requestDraftDocuments.createdAt));
}

export async function removeDocumentFromDraft(input: {
  ownerUserId: number;
  conversationId: string;
  documentId: number;
}) {
  ...
  if (!session?.draft) throw new Error("DRAFT_NOT_FOUND");
  await db.delete(requestDraftDocuments).where(
    and(
      eq(requestDraftDocuments.draftId, session.draft.id),
      eq(requestDraftDocuments.documentId, input.documentId),
      inArray(
        requestDraftDocuments.documentId,
        (
          await db
            .select({ id: documents.id })
            .from(documents)
            .where(eq(documents.ownerUserId, input.ownerUserId))
        ).map((item) => item.id),
      ),
    ),
  );
...
export async function requestHumanHandoff(input: {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(handoffRequests.ownerUserId, input.ownerUserId),
        eq(handoffRequests.conversationId, input.conversationId),
        inArray(handoffRequests.status, ["pending", "assigned"]),
      ),
    )
    .limit(1);
  if (existing[0])
  ...
  const ticket = await createSupportTicket({
    customerUserId: input.ownerUserId,
    subject: handoffSubject(input.language),
    priority,
    initialMessage: input.reason,
  });
  const handoffId = crypto.randomUUID();
  await db.transaction(async (tx) => {
    await tx.insert(handoffRequests).values({
      id: handoffId,
      ownerUserId: input.ownerUserId,
      conversationId: input.conversationId,
      draftId: session.draft?.id ?? undefined,
      ticketId: ticket.id,
      reason: input.reason,
      priority,
      status: "pending",
    ...
    await tx
      .update(aiConversations)
      .set({
        currentState: "needs_human_review",
    ...
    await tx.insert(notifications).values({
      ...
      title:
        input.language === "ar"
          ? "تم تحويل طلبك إلى فريق المتابعة"
          : "Your request was handed to the follow-up team",
      body:
        input.language === "ar"
          ? "سيظهر الرد والتحديث في مركز الدعم داخل التطبيق."
          : "Replies and updates will appear in the in-app support center.",
      type: "human_handoff",
...
export async function createAutomationEvent(input: {
  ...
  const existing = await db
    ...
    .where(eq(automationEvents.idempotencyKey, input.idempotencyKey))
    .limit(1);
...
export async function skipAutomationRun(runId: string, result: unknown) {
  ...
  await db
    .update(automationRuns)
    .set({ status: "skipped", result, completedAt: new Date() })
    .where(eq(automationRuns.id, runId));
}
...
export async function getAutomationOperationsDashboard() {
  ...
  const [rules, runs, pendingHandoffRows] = await Promise.all([
    ...
    db
      ...
      .innerJoin(
        automationEvents,
        eq(automationRuns.eventId, automationEvents.id),
      )
    ...
    db
      ...
      .from(handoffRequests)
      .where(inArray(handoffRequests.status, ["pending", "assigned"]))
      .limit(500),
  ]);
  return {
    rules,
    ...
    metrics: {
      ...
      pendingHandoffs: pendingHandoffRows.length,
    },
  };
}
...
export async function createAutomatedTask(input: {
  ...
  const exists = await db
    ...
    .where(
      and(
        eq(tasks.transactionId, input.transactionId),
        eq(tasks.title, input.title),
        eq(tasks.ownerUserId, input.ownerUserId),
        eq(tasks.status, "new"),
      ),
    )
    .limit(1);
  ...
  const result = await db.insert(tasks).values({
    ...
    priority: input.priority,
    status: "new",
    description: `Automation rule: ${input.ruleKey}`,
  });
...
export async function listTaskTrackingForUser(userId: number) {
  ...
  return db
    ...
    .where(or(eq(tasks.ownerUserId, userId), eq(tasks.assigneeUserId, userId)))
    .orderBy(asc(tasks.slaDueAt), desc(tasks.createdAt));
}

type TaskBlockReason = {
  type: "dependency" | "checklist" | "approval";
  count: number;
};
...
async function taskCompletionBlockReason(
  taskId: number,
): Promise<TaskBlockReason | undefined> {
  const db = await getDb();
  if (!db) return undefined;
  const dependencies = await db
    ...
    .where(
      and(eq(taskDependencies.taskId, taskId), ne(tasks.status, "completed")),
    );
  if (dependencies.length)
    return { type: "dependency", count: dependencies.length };
  const checklist = await db
    ...
    .where(
      and(
        ...
        eq(taskChecklistItems.isRequired, true),
        isNull(taskChecklistItems.completedAt),
      ),
    );
...
async function wouldCreateDependencyCycle(
  ...
  while (pending.length) {
    ...
    visited.add(current);
    edges
      .filter((edge) => edge.taskId === current)
      .forEach((edge) => pending.push(edge.dependsOnTaskId));
  }
  return false;
}

export async function getTaskExecutionDetailsForUser(
  userId: number,
  taskId: number,
) {
  const db = await getDb();
  ...
  const [dependencies, checklist] = await Promise.all([
    ...
    db
      .select({
        ...
        isRequired: taskChecklistItems.isRequired,
  ...
  return {
    dependencies,
    checklist,
    blockedBy: await taskCompletionBlockReason(taskId),
  };
}
...
export async function addTaskDependency(input: {
  ...
  if (input.taskId === input.dependsOnTaskId)
    throw new Error("TASK_DEPENDENCY_SELF_REFERENCE");
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [task, dependency] = await Promise.all([
    canAccessTrackedTask(input.userId, input.taskId),
    canAccessTrackedTask(input.userId, input.dependsOnTaskId),
  ]);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "";
    if (message.includes("Duplicate") || message.includes("ER_DUP_ENTRY"))
      throw new Error("TASK_DEPENDENCY_ALREADY_EXISTS");
    throw error;
  }
}
...
export async function addTaskChecklistItem(input: {
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  if (!(await canAccessTrackedTask(input.userId, input.taskId)))
...
export async function setTaskChecklistItemCompletion(input: {
  userId: number;
  ...
  const rows = await db
    ...
    .limit(1);
  const item = rows[0];
  if (!item || !(await canAccessTrackedTask(input.userId, item.taskId)))
    return false;
  const result = await db
    .update(taskChecklistItems)
    .set({
      completedAt: input.completed ? new Date() : null,
      completedByUserId: input.completed ? input.userId : null,
    })
    .where(eq(taskChecklistItems.id, input.checklistItemId));
  return (
    Number(
...
async function approvalResourceOwner(
  userId: number,
  resourceType: ApprovalResourceType,
  resourceId: string,
) {
  ...
  const rows = await db
    .select({ customerUserId: serviceRequests.customerUserId })
    .from(serviceRequests)
    .where(
      and(
        eq(serviceRequests.id, numericId),
        eq(serviceRequests.customerUserId, userId),
      ),
    )
    .limit(1);
...
export async function createApprovalRequest(input: {
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const ownerUserId = await approvalResourceOwner(
    input.userId,
    input.resourceType,
    input.resourceId,
  );
...
export async function listApprovalsForResource(
  userId: number,
  resourceType: ApprovalResourceType,
  resourceId: string,
) {
  const db = await getDb();
  if (!db || !(await approvalResourceOwner(userId, resourceType, resourceId)))
    return undefined;
  ...
  const steps = ids.length
    ? await db
        .select()
        .from(approvalSteps)
        .where(inArray(approvalSteps.approvalRequestId, ids))
        .orderBy(asc(approvalSteps.stepOrder))
    : [];
  return requests.map((request) => ({
    ...request,
    steps: steps.filter((step) => step.approvalRequestId === request.id),
  }));
}
...
export async function listPendingApprovalsForApprover(
  ...
  const candidates = await db
    .select({ request: approvalRequests, step: approvalSteps })
    .from(approvalSteps)
    .innerJoin(
      approvalRequests,
      eq(approvalSteps.approvalRequestId, approvalRequests.id),
    )
    .where(
      and(
        eq(approvalRequests.status, "pending"),
        ...
        or(
          eq(approvalSteps.assignedUserId, userId),
          eq(approvalSteps.requiredRole, userRole),
        ),
      ),
    )
  ...
  const allSteps = await db
    .select()
    .from(approvalSteps)
    .where(inArray(approvalSteps.approvalRequestId, requestIds))
    .orderBy(asc(approvalSteps.stepOrder));
  const taskIds = candidates
    .filter((row) => row.request.resourceType === "task")
    .map((row) => Number(row.request.resourceId))
    .filter((id) => Number.isInteger(id) && id > 0);
  const serviceRequestIds = candidates
    .filter((row) => row.request.resourceType === "service_request")
    .map((row) => Number(row.request.resourceId))
    .filter((id) => Number.isInteger(id) && id > 0);
  const taskRows = taskIds.length
    ? await db
    ...
    : [];
  const serviceRequestRows = serviceRequestIds.length
    ? await db
        .select({
          id: serviceRequests.id,
          title: serviceRequests.title,
          desiredDueAt: serviceRequests.desiredDueAt,
        })
        .from(serviceRequests)
        .where(inArray(serviceRequests.id, [...new Set(serviceRequestIds)]))
    : [];
  const taskLabels = new Map(
    taskRows.map((task) => [
      String(task.id),
      { label: task.title, dueAt: task.slaDueAt ?? task.dueAt ?? null },
    ]),
  );
  const serviceRequestLabels = new Map(
    serviceRequestRows.map((request) => [
      String(request.id),
      { label: request.title, dueAt: request.desiredDueAt ?? null },
    ]),
  );
  const actionable = candidates
    .filter(
      ({ request, step }) =>
        request.routingMode !== "sequential" ||
        allSteps
          .filter(
            (candidate) =>
    ...
    .map(({ request, step }) => {
      const resource =
        request.resourceType === "task"
          ? taskLabels.get(request.resourceId)
          : serviceRequestLabels.get(request.resourceId);
      return {
        ...
        resourceLabel:
          resource?.label ??
          (request.resourceType === "task"
            ? `مهمة #${request.resourceId}`
            : `طلب خدمة #${request.resourceId}`),
        dueAt: resource?.dueAt ?? null,
      };
    });
  const now = new Date();
  const filtered = actionable.filter((item) => {
    if (options.resourceType && item.resourceType !== options.resourceType)
      return false;
...
export async function decideApprovalStep(input: {
  ...
  const steps = await db
    .select()
    .from(approvalSteps)
    .where(eq(approvalSteps.approvalRequestId, request.id))
    .orderBy(asc(approvalSteps.stepOrder));
  ...
  if (
    step.assignedUserId
      ? step.assignedUserId !== input.userId
      : step.requiredRole !== input.userRole
  )
    throw new Error("APPROVAL_DECISION_FORBIDDEN");
  if (
    request.routingMode === "sequential" &&
    steps.some(
      (candidate) =>
    ...
    throw new Error("APPROVAL_SEQUENCE_BLOCKED");
  await db
  ...
  const nextStatus =
    ...
    steps
      ...
      .every((candidate) => ["approved", "skipped"].includes(candidate.status))
      ? "approved"
      : input.decision === "approved"
        ? "pending"
        : input.decision;
...
export async function updateTrackedTaskStatus(input: {
  userId: number;
  taskId: number;
  status:
    | "new"
  ...
  const blockedBy =
    input.status === "completed"
      ? await taskCompletionBlockReason(input.taskId)
      : undefined;
  ...
  const result = await db
    ...
    .where(
      and(
        ...
        or(
          eq(tasks.ownerUserId, input.userId),
          eq(tasks.assigneeUserId, input.userId),
        ),
      ),
    );
  return {
...
export async function ensureInitialZatcaOfficialSource(actorUserId: number) {
  ...
  if (!db) throw new Error("Database not available");
  const existing = await db
    .select({ id: officialSources.id })
    .from(officialSources)
    .where(eq(officialSources.feedUrl, ZATCA_OFFICIAL_SOURCE.feedUrl))
    .limit(1);
  ...
  await createAuditLog({
    actorUserId,
    action: "official_source.initial_zatca_created",
    resourceType: "official_source",
    resourceId: id,
    metadata: { feedUrl: ZATCA_OFFICIAL_SOURCE.feedUrl },
  });
...
export async function listOfficialSourcesForAdmin() {
  ...
  return db
    .select()
    .from(officialSources)
    .orderBy(desc(officialSources.updatedAt));
}

export async function collectOfficialSource(input: {
  actorUserId?: number;
  sourceId: number;
}) {
  const db = await getDb();
  ...
  const sourceRows = await db
    .select()
    .from(officialSources)
    .where(eq(officialSources.id, input.sourceId))
    .limit(1);
  ...
  if (
    ...
    throw new Error("OFFICIAL_SOURCE_NOT_COLLECTABLE");
  if (
    !isOfficialGovernmentUrl(source.officialUrl) ||
    !isOfficialGovernmentUrl(source.feedUrl)
  )
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(`OFFICIAL_SOURCE_FETCH_${response.status}`);
    ...
    for (const item of items) {
      const existing = await db
        ...
        .where(
          and(
            eq(regulatoryUpdates.sourceId, source.id),
            eq(regulatoryUpdates.checksum, item.checksum),
          ),
        )
        .limit(1);
      if (existing[0]) {
    ...
    await db
      ...
      .set({ lastCheckedAt: now, lastSuccessAt: now })
      .where(eq(officialSources.id, source.id));
    await createAuditLog({
      actorUserId: input.actorUserId,
      action: "official_source.collected",
      resourceType: "official_source",
      resourceId: source.id,
      metadata: { created, duplicates, itemCount: items.length },
    });
    ...
    await createAuditLog({
      actorUserId: input.actorUserId,
      action: "official_source.collection_failed",
      resourceType: "official_source",
      resourceId: source.id,
      metadata: {
        reason:
...
export async function collectVerifiedOfficialSources(actorUserId?: number) {
  ...
  return {
    ...
    created,
    duplicates,
    failedSourceIds: failures,
  };
}

export async function listRegulatoryUpdatesForAdmin(
  ...
  const rows = await db
    .select({
      ...
      sourceName: officialSources.sourceName,
      authorityNameAr: officialSources.authorityNameAr,
    })
    .from(regulatoryUpdates)
    .innerJoin(
      officialSources,
      eq(regulatoryUpdates.sourceId, officialSources.id),
    )
...
export async function listPublishedRegulatoryUpdates() {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(regulatoryUpdates.status, "published"),
        isNull(regulatoryUpdates.deletedAt),
      ),
    )
    .orderBy(
      desc(regulatoryUpdates.publishedAtSystem),
      desc(regulatoryUpdates.publishedAt),
    )
    .limit(100);
  return rows.map((row) => ({
    id: row.update.id,
...
export async function reviewRegulatoryUpdate(input: {
  actorUserId: number;
  updateId: number;
  ...
  const rows = await db
    .select()
    .from(regulatoryUpdates)
    .where(eq(regulatoryUpdates.id, input.updateId))
    .limit(1);
  const update = rows[0];
  if (!update || update.deletedAt)
    throw new Error("REGULATORY_UPDATE_NOT_FOUND");
  const now = new Date();
  const patch = {
  ...
  if (input.action === "verify") {
    if (!mayTransitionRegulatoryUpdate(update.status, input.action))
      throw new Error("REGULATORY_UPDATE_REVIEW_STATE_INVALID");
    await db
      .update(regulatoryUpdates)
      .set({ ...patch, status: "verified" })
      .where(eq(regulatoryUpdates.id, update.id));
  } else if (input.action === "publish") {
    if (!mayTransitionRegulatoryUpdate(update.status, input.action))
      throw new Error("REGULATORY_UPDATE_NOT_VERIFIED");
    await db
      .update(regulatoryUpdates)
      .set({
        ...patch,
    ...
    await db
      .update(regulatoryUpdates)
      .set({ ...patch, status: "rejected" })
      .where(eq(regulatoryUpdates.id, update.id));
  }
  await createAuditLog({
    actorUserId: input.actorUserId,
    action: `regulatory_update.${input.action}`,
    resourceType: "regulatory_update",
    resourceId: update.id,
    metadata: {
      priorStatus: update.status,
...
export async function listUpdateSubscriptionsForUser(userId: number) {
  ...
  return db
    ...
    .where(eq(updateSubscriptions.userId, userId))
    .orderBy(desc(updateSubscriptions.updatedAt));
}

export async function createUpdateSubscription(input: {
  userId: number;
  sourceId?: number;
  updateType?: string;
  activity?: string;
  ...
  if (!db) throw new Error("Database not available");
  const result = await db
    .insert(updateSubscriptions)
    .values({ ...input, isActive: true });
  return { id: Number(result[0].insertId) };
}

export async function setUpdateSubscriptionActive(input: {
  userId: number;
  ...
  const result = await db
    ...
    .where(
      and(
        eq(updateSubscriptions.id, input.subscriptionId),
        eq(updateSubscriptions.userId, input.userId),
      ),
    );
  return (
    Number(
