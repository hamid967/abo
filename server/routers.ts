...
import {
  ...
  guideRequestIntake,
  requestIntakeStageSchema,
} from "./abu-mishal-assistant";
import {
  detectRequestIntent,
  draftPatchFromDetection,
...
const beneficiaryTypeSchema = z.enum([
  "individual",
  "establishment",
  "company",
  "association",
...
const cloudRecordTypeSchema = z.enum([
  "transactions",
  "workspace",
  "inquiries",
  "today-actions",
...
const verifiedDownloadUrlSchema = z
  .string()
  .url()
  .max(2048)
  .refine(
    (value) => new URL(value).protocol === "https:",
    "DOWNLOAD_URL_MUST_USE_HTTPS",
  );
...
export const appRouter = router({
  ...
  voice: router({
    transcribeIntake: protectedProcedure
      .input(
        z.object({
          audioBase64: z.string().min(16).max(7_000_000),
          mimeType: z.enum(voiceIntakeMimeTypes),
          language: z.enum(["ar", "en"]).default("ar"),
        }),
      )
      .mutation(async ({ ctx, input }) => {
        const allowance = checkAssistantRateLimit({
          userId: ctx.user.id,
          action: "voice",
        });
  ...
  requests: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listServiceRequests(ctx.user.id, ctx.user.role),
    ),
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (
          ...
          !(await db.canUseOrganization(ctx.user.id, input.organizationId))
        ) {
          throw new TRPCError({
            code: "FORBIDDEN",
  ...
  assistant: router({
    ask: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const response = await answerGuidanceQuestion(
          input.question,
          input.language,
        );
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.guidance_question",
    ...
    intakeGuide: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.request_intake_guidance",
          resourceType: "assistant",
          metadata: { stage: input.stage, messageLength: input.message.length },
        });
        return response;
      }),
  }),
    ...
    analyze: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!allowance.allowed)
          throw new TRPCError({
            code: "TOO_MANY_REQUESTS",
            message: "DOCUMENT_SUMMARY_RATE_LIMITED",
          });
        try {
          const summary = await summarizeDocumentText({
            text: input.text,
            language: input.language,
          });
          await db.createAuditLog({
            actorUserId: ctx.user.id,
            action: "document_summary.analyzed",
            resourceType: "document_summary",
            metadata: {
          ...
          throw new TRPCError({
            ...
            message: "DOCUMENT_SUMMARY_UNAVAILABLE",
          });
        }
      }),
  }),
  executiveAssistant: router({
    start: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!allowance.allowed)
          throw new TRPCError({
            code: "TOO_MANY_REQUESTS",
            message: "ASSISTANT_RATE_LIMITED",
          });
        const session = await db.createRequestConversation({
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.conversation_started",
          resourceType: "ai_conversation",
          resourceId: session.conversation?.id,
          metadata: { reused: session.reused },
        });
        return session;
      }),
    detail: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        const session = await db.getRequestConversation(
          ctx.user.id,
    ...
    listDrafts: protectedProcedure.query(({ ctx }) =>
      db.listActiveRequestDrafts(ctx.user.id),
    ),
    serviceSuggestions: protectedProcedure
      .input(
        z.object({
          term: z.string().trim().min(2).max(180),
          limit: z.number().int().min(1).max(10).default(6),
        }),
      )
      .query(({ input }) => db.searchActiveServices(input.term, input.limit)),
    updateDraft: protectedProcedure
      .input(
        z.object({
          conversationId: z.string().uuid(),
          patch: requestDraftPatchSchema,
        }),
      )
      .mutation(async ({ ctx, input }) => {
        if (
          input.patch.organizationId &&
          !(await db.canUseOrganization(
            ctx.user.id,
            input.patch.organizationId,
          ))
        ) {
        ...
        const session = await db.updateRequestDraftFields({
          ownerUserId: ctx.user.id,
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.draft_updated",
          resourceType: "request_draft",
          resourceId: session?.draft?.id,
    ...
    validateDraft: protectedProcedure
      .input(z.object({ conversationId: z.string().uuid() }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.validateRequestDraft(
          ctx.user.id,
          input.conversationId,
        );
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.draft_validated",
          resourceType: "ai_conversation",
    ...
    transition: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          return await db.moveConversationState(
            ctx.user.id,
            input.conversationId,
            input.nextState,
          );
          ...
          if (
            error instanceof Error &&
            error.message.startsWith("INVALID_CONVERSATION_TRANSITION")
          )
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Invalid conversation state transition.",
            });
          throw error;
        }
      }),
    prepareReview: protectedProcedure
      .input(z.object({ conversationId: z.string().uuid() }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.prepareDraftReview(
          ctx.user.id,
          input.conversationId,
        );
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.review_prepared",
          resourceType: "request_draft",
          resourceId: result?.draft?.id,
          metadata: { summaryVersion: result?.draft?.summaryVersion },
        });
        return result;
      }),
    recordConsent: protectedProcedure
      .input(
        z.object({
          conversationId: z.string().uuid(),
          consentType: z.enum(["terms", "privacy", "request_submission"]),
        }),
      )
      .mutation(async ({ ctx, input }) => {
        const result = await db.recordDraftConsent({
          ownerUserId: ctx.user.id,
          ...input,
        });
        await db.createAuditLog({
    ...
    submitDraft: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.submitRequestDraft(
            ctx.user.id,
            input.conversationId,
            input.language,
          );
          if (!result.alreadySubmitted && result.transactionId)
            await emitAndProcessAutomationEvent({
              eventName: "request.created",
              aggregateType: "service_request",
              aggregateId: String(result.requestId),
              ownerUserId: ctx.user.id,
              payload: {
                requestId: result.requestId,
                transactionId: result.transactionId,
                automationOrigin: false,
              },
              idempotencyKey: `request-created:${result.requestId}`,
            });
          return result;
        } catch (error) {
          const message =
    ...
    listDraftDocuments: protectedProcedure
      .input(z.object({ conversationId: z.string().uuid() }))
      .query(({ ctx, input }) =>
        db.listDraftDocuments(ctx.user.id, input.conversationId),
      ),
    attachDocument: protectedProcedure
      .input(
        z.object({
          ...
          requirementKey: z.string().trim().max(120).optional(),
        }),
      )
      .mutation(async ({ ctx, input }) => {
        const result = await db.attachDocumentToDraft({
          ownerUserId: ctx.user.id,
          ...input,
        });
        await emitAndProcessAutomationEvent({
          eventName: "draft.document_attached",
          aggregateType: "ai_conversation",
          aggregateId: input.conversationId,
          ownerUserId: ctx.user.id,
          payload: { documentId: input.documentId, automationOrigin: false },
          idempotencyKey: `draft-document:${input.conversationId}:${input.documentId}`,
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.draft_document_attached",
          resourceType: "ai_conversation",
          resourceId: input.conversationId,
    ...
    removeDocument: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await db.removeDocumentFromDraft({
          ownerUserId: ctx.user.id,
          ...input,
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.draft_document_removed",
          resourceType: "ai_conversation",
          resourceId: input.conversationId,
          metadata: { documentId: input.documentId },
        });
    ...
    cancelDraft: protectedProcedure
      .input(z.object({ conversationId: z.string().uuid() }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.cancelRequestConversation(
          ctx.user.id,
          input.conversationId,
        );
        await db.createAuditLog({
    ...
    deleteConversationData: protectedProcedure
      .input(z.object({ conversationId: z.string().uuid() }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.deleteAssistantConversationContent(
          ctx.user.id,
          input.conversationId,
        );
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.conversation_content_deleted",
          resourceType: "ai_conversation",
          resourceId: input.conversationId,
          metadata: {
            submittedRequestPreserved: result.submittedRequestPreserved,
          },
        });
        return result;
      }),
    requestHumanHandoff: protectedProcedure
      .input(
        z.object({
          conversationId: z.string().uuid(),
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await db.requestHumanHandoff({
          ownerUserId: ctx.user.id,
          ...input,
        });
        if (!result.reused)
          await emitAndProcessAutomationEvent({
            ...
            idempotencyKey: `handoff-requested:${result.handoffId}`,
          });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "assistant.human_handoff_requested",
          resourceType: "ai_conversation",
          resourceId: input.conversationId,
          metadata: {
            handoffId: result.handoffId,
    ...
    sendMessage: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!session?.conversation) throw new TRPCError({ code: "NOT_FOUND" });
        try {
          const detection = await detectRequestIntent({
          ...
          const matchedService = detection.entities.serviceName
            ? await db.findMatchingActiveService(detection.entities.serviceName)
            : undefined;
          const enrichedPatch = matchedService
            ? { ...draftPatch, serviceId: matchedService.id }
            : draftPatch;
          const updatedSession = Object.keys(enrichedPatch).length
            ? await db.updateRequestDraftFields({
                ownerUserId: ctx.user.id,
                conversationId: input.conversationId,
                patch: enrichedPatch,
              })
            : session;
          if (session.conversation.currentState === "started")
            await db.saveConversationProgress({
              ownerUserId: ctx.user.id,
              conversationId: input.conversationId,
              nextState: "identifying_intent",
            });
          const structuredData =
            updatedSession?.draft?.structuredData &&
            typeof updatedSession.draft.structuredData === "object" &&
            !Array.isArray(updatedSession.draft.structuredData)
              ? (updatedSession.draft.structuredData as Record<string, unknown>)
              : {};
          const intake = nextTransactionIntakeQuestion(
            structuredData,
            input.language,
          );
          const guidance =
            detection.intent === "ask_requirements"
              ? await answerGuidanceQuestion(input.message, input.language)
              : undefined;
          const reply = detection.requiresHumanReview
            ? input.language === "ar"
              ? "شكرًا، سيتولى فريق المتابعة مراجعة هذا الطلب معك. لن ننفذ أي إجراء حساس عبر المحادثة."
              : "Thank you. The support team will review this with you; no sensitive action will be completed in chat."
            : guidance
              ? `${guidance.answer}\n\n${intake.reply}`
              : input.language === "ar"
                ? `أبشر، سجّلت اللي ذكرته. ${intake.reply}`
                : `I recorded what you shared. ${intake.reply}`;
          const sources =
          ...
          await db.appendConversationMessage({
            ...
            metadata: {
              intent: detection.intent,
              nextField: intake.field,
              readyForReview: intake.readyForReview,
              sources,
          ...
          await db.createAuditLog({
            actorUserId: ctx.user.id,
            action: "assistant.intent_detected",
            resourceType: "ai_conversation",
            resourceId: input.conversationId,
            metadata: {
              intent: detection.intent,
              confidence: detection.confidence,
              humanReview: detection.requiresHumanReview,
            },
          });
          return {
            detection,
            ...
            draft: updatedSession?.draft ?? null,
          };
        } catch (error) {
          const message =
            error instanceof Error ? error.message : "INVALID_MESSAGE";
          if (
            message === "SENSITIVE_CONVERSATION_CONTENT" ||
            message === "INVALID_CHAT_MESSAGE"
          )
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Sensitive or invalid message content.",
            });
          throw error;
        }
      }),
  ...
  taskTracking: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listTaskTrackingForUser(ctx.user.id),
    ),
    detail: protectedProcedure
      .input(z.object({ taskId: z.number().int().positive() }))
      .query(async ({ ctx, input }) => {
        const result = await db.getTaskExecutionDetailsForUser(
          ctx.user.id,
          input.taskId,
        );
    ...
    addDependency: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.addTaskDependency({
            userId: ctx.user.id,
            ...input,
          });
          await db.createAuditLog({
          ...
          const message =
            error instanceof Error ? error.message : "TASK_DEPENDENCY_FAILED";
          if (
            [
    ...
    addChecklistItem: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.addTaskChecklistItem({
            userId: ctx.user.id,
            ...input,
          });
          await db.createAuditLog({
    ...
    setChecklistCompletion: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(async ({ ctx, input }) => {
        const updated = await db.setTaskChecklistItemCompletion({
          userId: ctx.user.id,
          ...input,
        });
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "task.checklist_item_completion_updated",
    ...
    updateStatus: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await db.updateTrackedTaskStatus({
          userId: ctx.user.id,
          ...input,
        });
        if (result.blockedBy)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              result.blockedBy.type === "dependency"
                ? "TASK_BLOCKED_BY_DEPENDENCY"
                : result.blockedBy.type === "checklist"
                  ? "TASK_BLOCKED_BY_CHECKLIST"
                  : "TASK_BLOCKED_BY_APPROVAL",
          });
  ...
  approvals: router({
    inbox: protectedProcedure
      ...
      .query(async ({ ctx, input }) =>
        db.listPendingApprovalsForApprover(ctx.user.id, ctx.user.role, input),
      ),
    forResource: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        const approvals = await db.listApprovalsForResource(
          ctx.user.id,
          input.resourceType,
          input.resourceId,
        );
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.createApprovalRequest({
            userId: ctx.user.id,
            ...input,
          });
          await db.createAuditLog({
            ...
            resourceId: Number(input.resourceId) || undefined,
            metadata: {
          ...
          if (
            error instanceof Error &&
            error.message === "APPROVAL_RESOURCE_NOT_FOUND"
          )
            throw new TRPCError({ code: "NOT_FOUND" });
          if (
            error instanceof Error &&
            error.message === "APPROVAL_REQUEST_ALREADY_PENDING"
          )
            throw new TRPCError({
              code: "BAD_REQUEST",
    ...
    decide: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.decideApprovalStep({
            userId: ctx.user.id,
          ...
          const message =
            error instanceof Error ? error.message : "APPROVAL_DECISION_FAILED";
          if (
            [
            ...
            throw new TRPCError({ code: "BAD_REQUEST", message });
          if (message === "APPROVAL_DECISION_FORBIDDEN")
  ...
  officialUpdates: router({
    listPublished: protectedProcedure.query(() =>
      db.listPublishedRegulatoryUpdates(),
    ),
    subscriptions: protectedProcedure.query(({ ctx }) =>
      db.listUpdateSubscriptionsForUser(ctx.user.id),
    ),
    subscribe: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) =>
        db.createUpdateSubscription({ userId: ctx.user.id, ...input }),
      ),
    setSubscriptionActive: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(async ({ ctx, input }) => {
        const updated = await db.setUpdateSubscriptionActive({
          userId: ctx.user.id,
          ...input,
        });
        if (!updated) throw new TRPCError({ code: "NOT_FOUND" });
    ...
    adminSources: protectedProcedure.query(async ({ ctx }) => {
      if (!canViewSystemDashboard(ctx.user.role))
    ...
    collect: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        try {
          return await db.collectOfficialSource({
            actorUserId: ctx.user.id,
          ...
          const message =
            error instanceof Error
              ? error.message
              : "OFFICIAL_SOURCE_COLLECTION_FAILED";
          if (
            [
              "OFFICIAL_SOURCE_NOT_COLLECTABLE",
          ...
          throw new TRPCError({
            code: "BAD_GATEWAY",
            message: "OFFICIAL_SOURCE_COLLECTION_FAILED",
          });
        }
      }),
    ...
    review: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        try {
          return await db.reviewRegulatoryUpdate({
            actorUserId: ctx.user.id,
            ...input,
          });
        } catch (error) {
          const message =
            error instanceof Error
              ? error.message
              : "REGULATORY_UPDATE_REVIEW_FAILED";
          if (["REGULATORY_UPDATE_NOT_FOUND"].includes(message))
            throw new TRPCError({ code: "NOT_FOUND", message });
          if (
            [
              "REGULATORY_UPDATE_REVIEW_STATE_INVALID",
              "REGULATORY_UPDATE_NOT_VERIFIED",
            ].includes(message)
          )
            throw new TRPCError({ code: "BAD_REQUEST", message });
          throw error;
  ...
  cloud: router({
    get: protectedProcedure
      .input(z.object({ recordType: cloudRecordTypeSchema }))
      .query(async ({ ctx, input }) => {
        const record = await db.getCloudRecord(ctx.user.id, input.recordType);
        return record
          ? { payload: record.payload, updatedAt: record.updatedAt }
          : null;
      }),
    put: protectedProcedure
      .input(
        z.object({ recordType: cloudRecordTypeSchema, payload: z.unknown() }),
      )
      .mutation(({ ctx, input }) => {
        if (!isCloudPayloadWithinLimit(input.payload))
          throw new TRPCError({
            code: "PAYLOAD_TOO_LARGE",
            message: "Cloud record exceeds the allowed size.",
          });
        return db.upsertCloudRecord(
          ctx.user.id,
          input.recordType,
          input.payload,
        );
  ...
  documents: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listOwnedDocuments(ctx.user.id),
    ),
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          bytes.length > input.fileSizeBytes + 2048
        )
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        const { key, url } = await storagePut(
          `abu-mishal/${ctx.user.id}/documents/${Date.now()}-${safeName}`,
          bytes,
          input.mimeType,
        );
        const id = await db.createUploadedDocument({
          ownerUserId: ctx.user.id,
        ...
        await db.createAuditLog({
          ...
          resourceId: id,
          metadata: { mimeType: input.mimeType, fileSizeBytes: bytes.length },
        });
    ...
    downloadUrl: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const document = await db.getOwnedDocumentForAccess(
          ctx.user.id,
          input.documentId,
        );
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "document.download_link_requested",
          resourceType: "document",
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.softDeleteOwnedDocument(
            ctx.user.id,
            input.documentId,
          );
          await db.createAuditLog({
          ...
          if (
            ["DOCUMENT_NOT_FOUND", "DOCUMENT_LINKED_TO_RECORD"].includes(
              message,
            )
          )
            throw new TRPCError({ code: "BAD_REQUEST", message });
    ...
    extractFields: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!allowance.allowed)
          throw new TRPCError({
            code: "TOO_MANY_REQUESTS",
            message: "DOCUMENT_ANALYSIS_RATE_LIMITED",
          });
        const document = await db.getOwnedDocumentForAccess(
          ctx.user.id,
          input.documentId,
        ...
        if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "DOCUMENT_IMAGE_REQUIRED",
          });
        try {
          const extraction = await extractDocumentFields({
            imageUrl: await storageGetSignedUrl(document.storageKey),
            language: input.language,
          });
          const preview = await db.createDocumentFieldExtraction({
            ownerUserId: ctx.user.id,
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "DOCUMENT_EXTRACTION_UNAVAILABLE",
          });
        }
      }),
    confirmExtractedFields: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await db.confirmDocumentFieldExtraction({
            ownerUserId: ctx.user.id,
            extractionId: input.extractionId,
          });
          await db.createAuditLog({
            actorUserId: ctx.user.id,
            action: "document.fields_confirmed",
            resourceType: "document",
          ...
          if (
            error instanceof Error &&
            error.message === "DOCUMENT_EXTRACTION_NOT_FOUND"
          )
            throw new TRPCError({ code: "NOT_FOUND" });
          throw error;
        }
  ...
  playbooks: router({
    activeForService: protectedProcedure
      .input(z.object({ serviceId: z.number().int().positive() }))
      .query(async ({ input }) =>
        db.getPublishedPlaybookForService(input.serviceId),
      ),
    ...
    detail: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const result = await db.getPlaybookVersionDetails(
          input.playbookId,
          input.versionId,
        );
        if (!result) throw new TRPCError({ code: "NOT_FOUND" });
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        try {
          const result = await db.createServicePlaybook({
            ...input,
          ...
          const message =
            error instanceof Error ? error.message : "PLAYBOOK_CREATE_FAILED";
          if (
            ["SERVICE_NOT_FOUND", "ACTIVE_PLAYBOOK_ALREADY_EXISTS"].includes(
              message,
            )
          )
            throw new TRPCError({ code: "BAD_REQUEST", message });
    ...
    createVersion: protectedProcedure
            ...
            .array(
              z.object({
                ...
                expectedDurationMinutes: z
                  ...
                  .max(43_200)
                  .optional(),
                slaMinutes: z.number().int().min(15).max(43_200).optional(),
              }),
            )
            .min(1)
            .max(40),
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const result = await db.createPlaybookVersion({
          ...input,
          createdByUserId: ctx.user.id,
        });
        await db.createAuditLog({
    ...
    publish: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        try {
          const result = await db.publishPlaybookVersion({
            ...input,
          ...
          const message =
            error instanceof Error ? error.message : "PLAYBOOK_PUBLISH_FAILED";
          if (
            [
              "PLAYBOOK_VERSION_NOT_FOUND",
              "ARCHIVED_VERSION_CANNOT_BE_PUBLISHED",
              "PLAYBOOK_STEPS_REQUIRED",
            ].includes(message)
          )
            throw new TRPCError({ code: "BAD_REQUEST", message });
          throw error;
  ...
  tickets: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listSupportTickets(ctx.user.id, ctx.user.role),
    ),
    detail: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new TRPCError({ code: "NOT_FOUND" });
        const messages = await db.listTicketMessages(
          ticket.id,
          canOperateTransactions(ctx.user.role),
        );
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.createInAppNotification({
          ...
          body: "سيتابع فريق أبو مشعل طلبك عبر مركز الدعم.",
          type: "support_ticket",
        ...
        await db.createAuditLog({
          ...
          metadata: { transactionId: input.transactionId ?? null },
        });
        return ticket;
      }),
    reply: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new TRPCError({ code: "NOT_FOUND" });
        if (input.isInternal && !canOperateTransactions(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const isOperator = canOperateTransactions(ctx.user.role);
        const message = await db.addTicketMessage({
          ...input,
          authorUserId: ctx.user.id,
          nextStatus: isOperator ? "awaiting_customer" : "in_progress",
        });
        if (isOperator && !input.isInternal)
          await db.createInAppNotification({
            recipientUserId: ticket.customerUserId,
            title: "رد جديد على تذكرة الدعم",
            body: `وصل رد بخصوص: ${ticket.subject}`,
            type: "support_ticket",
            data: { ticketId: ticket.id },
          });
        if (!isOperator && ticket.assignedUserId)
          await db.createInAppNotification({
            recipientUserId: ticket.assignedUserId,
            title: "رسالة جديدة من العميل",
            body: `تذكرة الدعم: ${ticket.subject}`,
            type: "support_ticket",
            data: { ticketId: ticket.id },
          });
        await db.createAuditLog({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canOperateTransactions(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const ticket = await db.getSupportTicketById(input.ticketId);
        ...
        if (update.status)
          await db.createInAppNotification({
            ...
            body: `التذكرة «${ticket.subject}» أصبحت: ${update.status}`,
            type: "support_ticket",
            data: { ticketId },
          });
        await db.createAuditLog({
  ...
  abuMishalChat: router({
    ...
    detail: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (
          !ticket ||
          ticket.channel !== "abu_mishal_chat" ||
          (!isAdmin && ticket.customerUserId !== ctx.user.id)
        )
          throw new TRPCError({ code: "NOT_FOUND" });
    ...
    send: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        let ticket = input.ticketId
          ? await db.getSupportTicketById(input.ticketId)
          : undefined;
        if (isAdmin) {
          if (!ticket || ticket.channel !== "abu_mishal_chat")
            throw new TRPCError({ code: "NOT_FOUND" });
          const message = await db.addTicketMessage({
            ticketId: ticket.id,
            authorUserId: ctx.user.id,
          ...
          await db.createInAppNotification({
            recipientUserId: ticket.customerUserId,
            title: "رد جديد من أبو مشعل",
            body: "وصلك رد جديد في محادثتك مع أبو مشعل.",
            type: "abu_mishal_chat",
            data: { ticketId: ticket.id },
          });
          await db.createAuditLog({
            actorUserId: ctx.user.id,
            action: "abu_mishal_chat.admin_message_sent",
            resourceType: "support_ticket",
            resourceId: ticket.id,
          });
        ...
        if (
          ticket &&
          (ticket.channel !== "abu_mishal_chat" ||
            ticket.customerUserId !== ctx.user.id)
        )
          throw new TRPCError({ code: "NOT_FOUND" });
        ...
        if (!ticket)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "تعذر بدء المحادثة حالياً.",
          });
        const message = await db.addTicketMessage({
          ticketId: ticket.id,
          authorUserId: ctx.user.id,
        ...
        await Promise.all(
          admins.map((admin) =>
            db.createInAppNotification({
              recipientUserId: admin.id,
              title: "رسالة جديدة إلى أبو مشعل",
              body: "وصلت رسالة جديدة من أحد العملاء في صندوق المحادثات.",
              type: "abu_mishal_chat",
              data: { ticketId: ticket!.id },
            }),
          ),
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "abu_mishal_chat.customer_message_sent",
          resourceType: "support_ticket",
          resourceId: ticket.id,
        });
        return { ...message, ticketId: ticket.id };
    ...
    markRead: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new TRPCError({ code: "NOT_FOUND" });
        return db.markAbuMishalChatRead({
          ticketId: ticket.id,
    ...
    updateStatus: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.createInAppNotification({
          recipientUserId: ticket.customerUserId,
          title: "تحديث على محادثة أبو مشعل",
          body: "تم تحديث حالة محادثتك مع أبو مشعل.",
          type: "abu_mishal_chat",
          data: { ticketId: ticket.id },
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "abu_mishal_chat.status_updated",
          resourceType: "support_ticket",
          resourceId: ticket.id,
          metadata: { status: input.status },
        });
  ...
  security: router({
    loginActivity: protectedProcedure.query(async ({ ctx }) => {
      ...
      await db.createAuditLog({
        actorUserId: ctx.user.id,
        action: "auth.login_activity_viewed",
        resourceType: "login_security",
      });
      return result;
    }),
    forgetDevice: protectedProcedure
      .input(z.object({ deviceId: z.string().min(32).max(64) }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.forgetLoginSecurityDevice(
          ctx.user.id,
          input.deviceId,
        );
        if (!result.removed) throw new TRPCError({ code: "NOT_FOUND" });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "auth.trusted_device_removed",
          resourceType: "login_security_device",
  ...
  notifications: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listNotificationCenter(ctx.user.id),
    ),
    markRead: protectedProcedure
  ...
  notificationPreferences: router({
    ...
    update: protectedProcedure
      .input(
        z.object({
          ...
          taskAlertsEnabled: z.boolean().optional(),
          calendarSyncEnabled: z.boolean().optional(),
          taskReminderMinutes: z.number().int().min(5).max(10_080).optional(),
          digestFrequency: z.enum(["immediate", "daily"]),
      ...
      .mutation(async ({ ctx, input }) => {
        const quiet = validateQuietHours({
          enabled: input.quietHoursEnabled,
          start: input.quietStartHour,
          end: input.quietEndHour,
        });
        if (!quiet.valid)
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        const result = await db.updateNotificationPreferences({
          userId: ctx.user.id,
          ...input,
    ...
    deliveryLog: protectedProcedure.query(({ ctx }) =>
      db.listNotificationDeliveryLogs(ctx.user.id),
    ),
  }),
  mobilePush: router({
    devices: protectedProcedure.query(({ ctx }) =>
      db.listMobilePushDevicesForUser(ctx.user.id),
    ),
    register: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await db.registerMobilePushDevice({
          userId: ctx.user.id,
          ...input,
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "mobile_push.device_registered",
          resourceType: "mobile_push_device",
          resourceId: result.id,
          metadata: { platform: input.platform, reused: result.reused },
        });
        return result;
      }),
    deactivate: protectedProcedure
      .input(z.object({ deviceId: z.string().trim().min(16).max(128) }))
      .mutation(async ({ ctx, input }) => {
        const result = await db.deactivateMobilePushDevice({
          userId: ctx.user.id,
          ...input,
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "mobile_push.device_deactivated",
          resourceType: "mobile_push_device",
          metadata: {},
        });
        return result;
  ...
  knowledge: router({
    list: protectedProcedure
      .input(z.object({ language: knowledgeLanguageSchema.default("ar") }))
      .query(({ input }) => db.listPublishedKnowledge(input.language)),
    createArticle: protectedProcedure
      .input(
        z.object({
          ...
          language: knowledgeLanguageSchema.default("ar"),
          sourceLabel: z.string().trim().max(255).optional(),
          sourceUrl: z.string().url().max(1024).optional(),
        }),
      )
      .mutation(async ({ ctx, input }) => {
        if (!canManageKnowledge(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const result = await db.createKnowledgeArticle({
          ...input,
          createdByUserId: ctx.user.id,
        });
        await db.createAuditLog({
    ...
    createFaq: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (!canManageKnowledge(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const result = await db.createFaqItem({
          ...input,
          createdByUserId: ctx.user.id,
        });
        await db.createAuditLog({
  ...
  search: router({
    records: protectedProcedure
      .input(z.object({ term: z.string().trim().min(2).max(120) }))
      .query(async ({ ctx, input }) => {
        const result = await db.searchAccessibleRecords(
          ctx.user.id,
          ctx.user.role,
          input.term,
        );
        await db.createAuditLog({
          ...
          resourceType: "search",
          metadata: { termLength: input.term.length },
        });
        return result;
      }),
  }),
  audit: router({
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        if (!canViewAuditLogs(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
  ...
  adminDashboard: router({
    overview: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const overview = await db.getSystemTransactionDashboard(
          input.status,
          input.search,
        );
        await db.createAuditLog({
    ...
    dailyDueStatus: protectedProcedure.query(async ({ ctx }) => {
      ...
      const stale =
        !lastRunAt ||
        Date.now() - new Date(lastRunAt).getTime() > 30 * 60 * 60 * 1000;
      await db.createAuditLog({
        actorUserId: ctx.user.id,
        action: "admin.daily_due_status_view",
        resourceType: "automation_schedule",
        resourceId: "daily_due_scan",
        metadata: { enabled: Boolean(schedule?.enabled), stale },
      });
      return {
    ...
    updateApprovalAlertSettings: protectedProcedure
      .input(
        z.object({
          approvalAlertWindowHours: z.union([
            z.literal(24),
            z.literal(48),
            z.literal(72),
          ]),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await db.updateAdminApprovalAlertSettings({
          ...input,
          updatedByUserId: ctx.user.id,
        });
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "admin.approval_alert_window_updated",
          resourceType: "admin_setting",
          resourceId: "approval_alert_window",
          metadata: {
    ...
    governanceGaps: protectedProcedure.query(async ({ ctx }) => {
      ...
      await db.createAuditLog({
        actorUserId: ctx.user.id,
        action: "admin.governance_gaps_viewed",
        resourceType: "governance_gap_dashboard",
        metadata: { auditBaseline: "phase_zero_2030" },
      });
  ...
  mobileReleases: router({
    ...
    save: protectedProcedure
      .input(
        z
          ...
          .superRefine((input, context) => {
            if (input.status === "ready" && !input.downloadUrl)
              context.addIssue({
                code: "custom",
                message: "READY_RELEASE_REQUIRES_DOWNLOAD_URL",
                path: ["downloadUrl"],
              });
          }),
      )
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          const releaseId = await db.saveMobileAppRelease({
            ...input,
            createdByUserId: ctx.user.id,
          });
          await db.createAuditLog({
            actorUserId: ctx.user.id,
            action: input.id
              ? "mobile_release.updated"
              : "mobile_release.created",
          ...
          if (
            error instanceof Error &&
            error.message === "MOBILE_RELEASE_NOT_FOUND"
          )
            throw new TRPCError({ code: "NOT_FOUND" });
          throw error;
        }
  ...
  automationOps: router({
    dashboard: protectedProcedure.query(async ({ ctx }) => {
      ...
      await db.createAuditLog({
        actorUserId: ctx.user.id,
        action: "automation.dashboard_view",
        resourceType: "automation_dashboard",
      });
      return result;
    }),
    setRuleEnabled: protectedProcedure
      .input(z.object({ ruleId: z.string().uuid(), enabled: z.boolean() }))
      .mutation(async ({ ctx, input }) => {
        if (!canViewSystemDashboard(ctx.user.role))
          throw new TRPCError({ code: "FORBIDDEN" });
        const result = await db.setAutomationRuleEnabled(
          input.ruleId,
          input.enabled,
        );
        await db.createAuditLog({
    ...
    previewRule: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.createAuditLog({
          actorUserId: ctx.user.id,
          action: "automation.rule_previewed",
          resourceType: "automation_rule",
          resourceId: input.ruleId,
          metadata: {
