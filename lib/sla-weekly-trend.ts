...
export function calculateSlaWeeklyTrend(tasks: ReadonlyArray<SlaWeeklyTrendTask>, now = new Date()): SlaWeeklyTrend {
  ...
  const points = Array.from({ length: 7 }, (_, index) => {
    ...
    const available = tasks.filter((task) => {
      ...
      return Boolean(createdAt && createdAt.getTime() < end.getTime() && (!completedAt || completedAt.getTime() >= day.getTime()));
    }).length;
    const completed = tasks.filter((task) => {
      const completedAt = dateValue(task.completedAt);
      return task.status === "completed" && completedAt && completedAt.getTime() >= day.getTime() && completedAt.getTime() < end.getTime();
    }).length;
    return { key: day.toISOString().slice(0, 10), label: formatter.format(day), completed, available, rate: percentage(completed, available) };
  });
  const weekStart = points.length ? new Date(`${points[0].key}T00:00:00`) : today;
  const weekEnd = dayEnd(today);
  const weeklyAvailable = tasks.filter((task) => {
    if (task.status === "cancelled") return false;
    const createdAt = dateValue(task.createdAt);
    const completedAt = dateValue(task.completedAt);
    return Boolean(createdAt && createdAt.getTime() < weekEnd.getTime() && (!completedAt || completedAt.getTime() >= weekStart.getTime()));
  }).length;
  const completedTotal = points.reduce((total, point) => total + point.completed, 0);
  ...
  return { points, weeklyRate: percentage(completedTotal, weeklyAvailable), completedTotal, direction: delta > 0 ? "up" : delta < 0 ? "down" : "steady", delta: Math.abs(delta) };
}
